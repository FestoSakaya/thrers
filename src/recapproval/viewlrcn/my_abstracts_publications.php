...
<input name="" type="button" class="search dropbtn2" value="Click to Submit Abstracts" onClick="window.location.href='./main.php?option=AddAbstracts&id=<?php echo $id;?>'"/>
...
<table width="100%" border="0" style="margin-top:15px;">
  <tr>
    <td width="59%">Find Abstracts & Publications:<br />
...
<div class="card-header d-flex align-items-center">
                      <h3 class="h4">Abstracts </h3>
</div>
                    <div class="card-body">
               <table class="table table-striped table-sm" id="customers">
                        <thead>
                          <tr>
                            <th width="146">Project Title </th>
                            <th width="159">Title</th>
                                      
                            <th width="42">Category</th>
                            <th width="134">Summary</th>
                          </tr>
                        </thead>
                        <tbody>
                        <?php
...
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."abstracts  where (details like '%$searchprotocol%') and owner_id='$asrmApplctID' order by id desc");//and conceptm_status='new' 
}
if(!$_POST['searchprotocol']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."abstracts  where owner_id='$asrmApplctID' order by id desc");//and conceptm_status='new' 
}
...
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS datesubmitted FROM ".$prefix."abstracts  where (details like '%$searchprotocol%') and owner_id='$asrmApplctID' order by id desc LIMIT $start, $limitm";//and conceptm_status='new'
}
if(!$_POST['searchprotocol']){							//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS datesubmitted FROM ".$prefix."abstracts  where owner_id='$asrmApplctID' order by id desc LIMIT $start, $limitm";//and conceptm_status='new'
}
...
$sqprotocol = $resultprotocol->fetch_array();
	...
	?>
                          <tr>
                            <td><h3 class="h4"><?php echo $sqprotocol['public_title'];?></h3></td>
                            <td><?php echo $rProjects['title'];?></td>
               
                            
                            <td><?php echo $rProjects['category'];?></td>
                            <td><?php echo $rProjects['details'];?></td>
                          </tr>
   <?php }///////////end function ?>                 
                        </tbody>
                      </table>
