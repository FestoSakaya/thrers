...
$cfnreviewer= $cfn_reviewer;
	...
	$meetingFor=$mysqli->real_escape_string($_POST['meetingFor']);
    //$protocol_id=$mysqli->real_escape_string($_POST['protocol_id']);
$protocol_id_annual=$mysqli->real_escape_string($_POST['protocol_id_annual'][$i]);
	
$sqlRStatus4 = "select * FROM ".$prefix."submission where protocol_id='$protocol_id_annual'";
$resultStatus4 = $mysqli->query($sqlRStatus4);
$rStatus4=$resultStatus4->fetch_array();
$public_title=$rStatus4['public_title'];
$recAffiliated_id=$rStatus4['recAffiliated_id'];
	
$sqlInvestigators_AnnualRenewal="SELECT * FROM ".$prefix."meeting where `subject`='$subject' and content='$comment' and protocol_id='$protocol_id_annual' and date>='$mdate'  and meetingFor='AnnualRenewal' order by id desc";
...
$sqlA2_AnnualRenewal="insert into ".$prefix."meeting (`created`,`updated`,`date`,`subject`,`content`,`protocol_id`,`public_title`,`recAffiliated_id`,`meetingFor`,`meetingCode`) 

values(now(),now(),'$mdate','$subject','$comment','$protocol_id_annual','$public_title','$recAffiliated_id','AnnualRenewal','$activ_code')";
...
$sqlRStatus4 = "select * FROM ".$prefix."submission where protocol_id='$protocol_id_ammendments'";
$resultStatus4 = $mysqli->query($sqlRStatus4);
$rStatus4=$resultStatus4->fetch_array();
$public_title=$rStatus4['public_title'];
$recAffiliated_id=$rStatus4['recAffiliated_id'];
	
$sqlInvestigators_Amendments="SELECT * FROM ".$prefix."meeting where `subject`='$subject' and content='$comment' and protocol_id='$protocol_id_ammendments' and date>='$mdate' and meetingFor='Amendments' order by id desc";
...
$sqlA2_Amendments="insert into ".$prefix."meeting (`created`,`updated`,`date`,`subject`,`content`,`protocol_id`,`public_title`,`recAffiliated_id`,`meetingFor`,`meetingCode`) 

values(now(),now(),'$mdate','$subject','$comment','$protocol_id_ammendments','$public_title','$recAffiliated_id','Amendments','$activ_code')";
...
$rStatus2=$resultStatus2->fetch_array();
	...
	?>
                          <tr>
                            <td><a href="<?php echo $base_url;?>recapproval/data/meetings/" onClick="return popitup('meetings.php?id=<?php echo $rProjects['date'];?>')"><?php echo $rProjects['date'];?></a> </td>
                            <td><a href="<?php echo $base_url;?>recapproval/data/meetings/" onClick="return popitup('meetings.php?id=<?php echo $rProjects['date'];?>')"><?php echo $rProjects['subject'];?></a></td>
                            <td><?php echo nl2br($rProjects['content']);?></td>
                            <td>
                          <?php
$sqlRStatus6 = "select * FROM ".$prefix."meeting where meetingCode='$meetingCode'";
...
$rStatus3=$resultStatus3->fetch_array();
  ...
  echo '<b>'.$countNo.'.</b> '.$rStatus3['public_title'].'<br><br>'; 
...
<span class="m-status m-status--wide cfx-watch"><a href="./data/ConductedMeetings/<?php echo $rProjects['meetingCode'];?>" onClick="return confirm('Are you sure you want to proceed?');">         <span class="cfx-watch-label">NOT CONDUCTED</span>
...
<span class="m-status m-status--wide cfx-watch active"><a href="./data/DidNotConductMeetings/<?php echo $rProjects['meetingCode'];?>" onClick="return confirm('Are you sure you want to proceed?');">        <span class="cfx-watch-label">CONDUCTED&nbsp;</span>
        <span class="cfx-watch-switch"></span></a></span><?php }?>
        
        
<?php if($session_privillage=='administrator' || $session_privillage=='superadmin' || $session_privillage=='recadmin'  || $session_privillage=='rechairperson' and $rProjects['date']>=$today){?><br />
  <input id="go" type="button" value="Re-schedule Meeting" onclick="window.open('<?php echo $base_url;?>recapproval/reschedulemeeting.php?id=<?php echo $rProjects['id'];?>','popUpWindow','height=700, width=1000, left=100, top=100, resizable=yes, scrollbars=yes, toolbar=yes, menubar=no, location=no, directories=no, status=yes');"  class="errorm4">
                            
    <br />
   <?php /*?><?php */?> <input id="go" type="button" value="Update This Meeting" onclick="window.open('<?php echo $base_url;?>recapproval/updatemeeting.php?id=<?php echo $rProjects['id'];?>','popUpWindow','height=700, width=1000, left=100, top=100, resizable=yes, scrollbars=yes, toolbar=yes, menubar=no, location=no, directories=no, status=yes');"  class="errorhona">
