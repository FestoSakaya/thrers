...
if($_POST['doFilesProceed']=='Save to Proceed' and $_POST['recAffiliated_id']){
...
$sqlprotocalSubSel3="SELECT * FROM ".$prefix."notifications where public_title='$public_title' and owner_id='$sessionasrmApplctID' and id='$id' order by id desc";
$QprotocalSub2Sel3 = $mysqli->query($sqlprotocalSubSel3);
$totalstudy3 = $QprotocalSub2Sel3->num_rows;

if(!$totalstudy3){
$sqlA22="insert into ".$prefix."notifications (`owner_id`,`protocol_id`,`recAffiliated_id`,`fileAttachment`,`created`,`status`,`assignedto`,`period`,`end_of_project`,`ammendType`,`public_title`) 

values('$sessionasrmApplctID','$project_id','$recAffiliated_id','$fileattachment',now(),'Pending','Not Assigned','','','manual','$public_title')";
$mysqli->query($sqlA22);
$record_id = $mysqli->insert_id;
$message='<div class="success">Changes have saved</div>';
echo '<meta http-equiv="REFRESH" content="0;url='.$base_url.'/main.php?option=ManualNotifications&id='.$record_id.'&rec='.$recAffiliated_id.'">';
}

if($totalstudy3 and $id){	
$sqlA2="update ".$prefix."notifications set `public_title`='$public_title' where owner_id='$sessionasrmApplctID' and id='$id'";
$mysqli->query($sqlA2);
//$message='<div class="success">Changes have saved</div>';
echo '<meta http-equiv="REFRESH" content="0;url='.$base_url.'/main.php?option=ManualNotificationsProceed&id='.$record_id.'&rec='.$recAffiliated_id.'">';
}
...
$sqlstudy="SELECT * FROM ".$prefix."notifications where `owner_id`='$sessionasrmApplctID'  and id='$id' order by id desc";
...
<input type="text" name="public_title" id="project_id" class="form-control  required" value="<?php echo $rNotification['public_title'];?>"/>
</div>
</div>
