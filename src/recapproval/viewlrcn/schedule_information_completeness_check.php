...
$sqlInvestigators="SELECT * FROM ".$prefix."submission_task where `owner_id`='$sasrmApplctID' and submission_id='$protocol_id' and description='$studyworkplan2' order by id desc";
...
values('$sasrmApplctID','$protocol_id',now(),now(),'$studyworkplan2','$startDate','$endDate')";
$mysqli->query($sqlA2);
...
$r= $cmdwb->fetch_array();
if($totalStages){
$sqlASubmissionStages="update ".$prefix."submission_stages  set `study_work_plan`='1' where `owner_id`='$sasrmApplctID' and `protocol_id`='$protocol_id'";
...
if($_POST['doWorkPlan']=='Save and Next'){
	echo '<meta http-equiv="REFRESH" content="0;url='.$base_url.'/main.php?option=submissionSixUp&id='.$id.'">';
		...
		//update edited table...
$sqlURevisions="SELECT * FROM ".$prefix."updated_sections where `owner_id`='$sasrmApplctID' and protocol_id='$protocol_id' and status='pending' order by id desc limit 0,1";
...
values('$sasrmApplctID','$protocol_id','','','','','','','','1','','','','',now(),'pending')";
$mysqli->query($sqlAREvisedSections);
}
if($totalUsersRevions){

$sqlAREvisedSections_update="update ".$prefix."updated_sections  set `study_work_plan`='1' where owner_id='$sasrmApplctID' and protocol_id='$protocol_id' and status='pending'";
...
<?php if($totalstudy3>=1){?><li><a href="./main.php?option=submissionCheck&id=<?php echo $rstudy['id'];?>" <?php if($rsSubStages['protocol_information']==1 and $rsSubStages['protocol_team']==1){?> style="background:green; color:#FFF;" <?php }?>>Protocol Information</a></li><?php }?>
...
<?php if($totalstudy3>=1){?><li><a href="./main.php?option=submissionSecondUp&id=<?php echo $rstudy3['id'];?>" <?php if($rsSubStages['protocol_details']==1){?> style="background:green; color:#FFF;" <?php }?>>Protocol Details</a></li><?php }?>
...
<?php if($totalstudy3>=1){?><li><a href="./main.php?option=submissionThirdUp&id=<?php echo $rstudy3['id'];?>" <?php if($rsSubStages['study_description']==1 and $rsSubStages['RecruitmentCountries']==1){?> style="background:green; color:#FFF;" <?php }?>>Study Description</a></li><?php }?>
...
<?php if($totalstudy3>=1){?><li><a href="./main.php?option=StudyPopulationUp&id=<?php echo $rstudy['id'];?>" <?php if($rsSubStages['study_population']==1){?> style="background:green; color:#FFF;" <?php }?>>Study Population</a></li><?php }?>
...
<?php if($totalstudy3>=1){?><li><a href="./main.php?option=submissionFourUp&id=<?php echo $rstudy['id'];?>" <?php if($rsSubStages['registry']==1){?> style="background:green; color:#FFF;" <?php }?>>Registry</a></li><?php }?>
<?php if(!$totalstudy3){?><li class="extra" <?php if($rsSubStages['registry']==1){?> style="background:green; color:#FFF;" <?php }?>>Registry</li><?php }}?>


<?php if($totalstudy3>=1){?><li><a href="./main.php?option=submissionBudgetUp&id=<?php echo $rstudy3['id'];?>" <?php if($rsSubStages['budget']==1){?> style="background:green; color:#FFF;" <?php }?>>Budget</a></li><?php }?>
...
<?php if($totalstudy3>=1){?><li><a href="./main.php?option=submissionSixUp&id=<?php echo $rstudy3['id'];?>" <?php if($rsSubStages['filem']==1){?> style="background:green; color:#FFF;" <?php }?>>Attached Files</a></li><?php }?>
<?php if(!$totalstudy3){?><li class="extra" <?php if($rsSubStages['filem']==1){?> style="background:green; color:#FFF;" <?php }?>>Attached Files</li><?php }?>

<?php if($totalstudy3>=1){?><li><a href="./main.php?option=submissionFinishUp&id=<?php echo $rstudy3['id'];?>" <?php if($rsSubStages['payments']==1){?> style="background:green; color:#FFF;" <?php }?>>Payments</a></li><?php }?>
...
<td>&nbsp;</td>
                        ...
                        <tbody>
<?php
if($category=='submissionScheduleDelUp' and $id){

	$sqlA2Protocol2="DELETE from ".$prefix."submission_task where owner_id='$asrmApplctID' and submission_id='$protocol_id' and id='$id'";
	$mysqli->query($sqlA2Protocol2);
	//check if empty rows
$sqlstudy22="SELECT * FROM ".$prefix."submission_task where `owner_id`='$asrmApplctID' and submission_id='$protocol_id' order by id desc limit 0,1";
$Querystudy33 = $mysqli->query($sqlstudy22);

if(!$Querystudy33->num_rows){
$sqlASubmissionStages="update ".$prefix."submission_stages  set `study_work_plan`='0' where `owner_id`='$asrmApplctID' and `protocol_id`='$protocol_id'";
$mysqli->query($sqlASubmissionStages);
}
	}
//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."submission_task where owner_id='$asrmApplctID' and submission_id='$protocol_id' order by id desc LIMIT 0,10";//and conceptm_status='new' 
$result = $mysqli->query($sql);
while($rInvestigator=$result->fetch_array()){
	?>
                          <tr>
                            <td><?php echo $rInvestigator['description'];?></td>
<td><a href="./main.php?option=submissionScheduleDelUp&id=<?php echo $rInvestigator['id'];?>" style="color:#F00; font-weight:bold;" onclick="return confirm('Are you sure you want to delete this item?');">Delete</a></td>
                          </tr>
   <?php }///////////end function ?>                 
                        </tbody>
                      </table>
