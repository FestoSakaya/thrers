...
?>
         <h2 class="no-margin-bottom">Declare Conflict of Interest</h2>
       ...
       <?php
//doSaveFive
if($_POST['doSaveConflict'] and $_POST['asrmApplctID'] and $id and $_POST['conflict']){

	$reason=$mysqli->real_escape_string($_POST['reason']);
	$reviewer_id=$mysqli->real_escape_string($_POST['asrmApplctID']);
	$conflict=$mysqli->real_escape_string($_POST['conflict']);
   $public_title=$mysqli->real_escape_string($_POST['public_title']);
//////////////update
$sqlA2ProtocolaSS="update ".$prefix."submission_review_sr  set `conflictofInterest`='$conflict' where id='$id' and asrmApplctID='$reviewer_id'";
...
$reviewerEmail=$sqReviewer['email'];

	
require("viewlrcn/class.phpmailer.php");
require("viewlrcn/class.smtp.php"); 	

require_once("viewlrcn/mail_template_conflict_of_interest.php");
...
/////////////////////////////Begin Mail Body
//$mail->addCc('mutumba.beth@yahoo.com',$ReviewerName);
//$mail->addBcc("uncstuncstapps@gmail.com",$ReviewerName);//$mail->addBcc($reviewerEmail,$ReviewerName);

$mail->FromName = "Reviewer - $ReviewerName"; //From Name -- CHANGE --
$mail->AddAddress($recemail, $ReviewerName); //To Address -- CHANGE --$email
//if($dbEmail2){echo $mail->addCc($dbEmail2,'Activation Link from UNCST');}
$mail->AddReplyTo($recemail, $ReviewerName); //Reply-To Address -- CHANGE --$usrm_email

$mail->WordWrap = 50; // set word wrap to 50 characters
$mail->IsHTML(false); // set email format to HTML
$mail->Subject = "REC Protocol Conflict of Interest - $public_title";
$body="$allSentMessage
";
$mail->MsgHTML($body);
...
<form action="" name="regForm" id="regForm" method="post" enctype="multipart/form-data" autocomplete="off">
 ...
 <label class="col-sm-10 form-control-label"><input name="conflict" type="radio" value="no" class="required"  onChange="getConflictofInterest(this.value)"> &nbsp;No conflict of Interest/Available to Review &nbsp;&nbsp;</label>
                            
<label class="col-sm-10 form-control-label"><input name="conflict" type="radio" value="yes" class="required"  onChange="getConflictofInterest(this.value)">&nbsp;Yes, I have Conflict of Interest</label>

<label class="col-sm-10 form-control-label"><input name="conflict" type="radio" value="not" class="required"  onChange="getConflictofInterest(this.value)">&nbsp;Not available to Review</label>
                            ...
                            <input type="hidden" name="public_title" value="<?php echo $rstudy['public_title'];?>">
 ...
 <div id="conflictdiv"></div>  
                    ...
                    <input name="doSaveConflict" type="submit"  class="btn btn-primary" value="Confirm"/>
