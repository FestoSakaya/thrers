...
if($_POST['doSubmitProceed']=='Save Details' and $_POST['protocol_title']){
...
$sqlprotocalSubSelCAllmm="SELECT * FROM ".$prefix."deviations where protocol_id='$project_id' and owner_id='$sessionasrmApplctID' and ammendType='manual' and is_sent='0' order by deviationID desc";
...
values('$sessionasrmApplctID','$project_id','$recAffiliated_id','$PDDateofoccurrence','$PDDescriptionofdeviation','$Rootcauseofdeviation','$Correctiveactiontaken','','$PVDateofoccurrence','$PVDescriptionofdeviation','$parta','$partaOther','$partb','$Rootcauseofviolation_b','$Correctiveaction_b','$Measurestomitigateviolation_b',now(),'Pending','Not Assigned','0','Not Paid','$attachpayment2','$protocol_title','manual','Pending','Pending','$chosen')";
$mysqli->query($sqlA2);
...
/*echo '<meta http-equiv="REFRESH" content="2;url=$base_url/main.php?option=DeviationsManual&id="'.$record_id.'">';*/

echo '<meta http-equiv="REFRESH" content="0;url='.$base_url.'/main.php?option=DeviationsManual&id='.$record_id.'">';
...
$mysqli->query($sqlA2);
$message='<p class="success">Dear '.$session_fullname.', details have been submitted, click finish button below to complete your submission</p>';
echo '<meta http-equiv="REFRESH" content="0;url='.$base_url.'/main.php?option=ChooseDeviationsManual&id='.$record_id.'">';	
}
...
<?php $owner_id=$_SESSION['asrmApplctID'];?>
  ...
  <textarea name="protocol_title" id="protocol_title" cols="" rows="5" class="form-control" required><?php echo $rstudy['protocol_title'];?></textarea>
