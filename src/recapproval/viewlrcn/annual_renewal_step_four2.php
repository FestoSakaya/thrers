...
<?php if($totalstudy>=1){?><li><a href="./main.php?option=<?php echo $link;?>&id=<?php echo $rstudy['id'];?>" <?php if($rsSubStages['protocol_information']==1){?> style="background:green; color:#FFF;" <?php }?>>Protocol Information</a></li><?php }?>
...
<?php
if($category=='FinalRenewalSubmit' and $_GET['id']){
	////Get Protocol ID
...
$email=$recUser['email'];
	...
	//FInish submission
$sqlA2Protocol1="update ".$prefix."renewals_summary  set `is_sent`='1' where `owner_id`='$sessionasrmApplctID' and annual_id='$id'";
...
$sqlASubmissionStages3="update ".$prefix."renewals_literature  set `is_sent`='1' where `owner_id`='$sessionasrmApplctID' and annual_id='$id'";
$mysqli->query($sqlASubmissionStages3);

$sqlASubmissionStages4="update ".$prefix."renewals  set `is_sent`='1',`status`='submitted',`CompletenessCheck`='Pending' where `owner_id`='$sessionasrmApplctID' and id='$id'";
...
echo '<div class="spacer"></div>';
 echo '<meta http-equiv="REFRESH" content="0;url='.$base_url.'/main.php?option=AnnualRenualMa">';
}
