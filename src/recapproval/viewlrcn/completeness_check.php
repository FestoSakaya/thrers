...
<li><a href="#" rel="#default" class="selected">Completeness Check</a></li>
</ul>
...
$sqlprotocalEdited="SELECT * FROM ".$prefix."updated_sections where  protocol_id='$protocol_idwe' and owner_id='$owner_id' and status='pending' order by id desc limit 0,1";
$QprotocalEdited = $mysqli->query($sqlprotocalEdited);
$rprotocalSEdited = $QprotocalEdited->fetch_array();
...
if($_POST['doSaveCommentsDraft']=='Save Comments' and $asrmApplctID and $_POST['status'] and $category=='CompletenessCheck'){
	$AssessorComments=$mysqli->real_escape_string(htmlentities($_POST['AssessorComments']));
	$status=$_POST['status'];
	///Chck if there was no update
$qcommentDrafts1="select * from ".$prefix."completeness_check_comments where protocol_id='$protocol_idwe' and reviewer_id='$asrmApplctID' and chdate='$today'";
$rcommentDrafts1=$mysqli->query($qcommentDrafts1);//assessorComment
if($rcommentDrafts1->num_rows){
	/////////////////////////
$Insert_send="update ".$prefix."completeness_check_comments set `chcomments`='$AssessorComments',`status`='$status' where protocol_id='$protocol_idwe' and reviewer_id='$asrmApplctID'";
$mysqli->query($Insert_send);

}
if(!$rcommentDrafts1->num_rows){
/////////////////////////
$Insert_send="insert into ".$prefix."completeness_check_comments (`owner_id`,`protocol_id`,`reviewer_id`,`chcomments`,`chdate`,`status`) values ('$owner_id','$protocol_idwe','$asrmApplctID','$AssessorComments',now(),'$status')";
$mysqli->query($Insert_send);
...
if($status=='Rejected' and $AssessorComments){

$sqlstudym33="SELECT * FROM ".$prefix."submission where id='$id' order by id desc limit 0,1";
$Querystudym33 = $mysqli->query($sqlstudym33);
$rstudym33 = $Querystudym33->fetch_array();

$Conditionalstatus=$rstudym33['status'];

if($Conditionalstatus=='Conditional Approval'){$newStatus='Conditional Approval';}
if($Conditionalstatus!='Conditional Approval'){$newStatus='completeness check';}

	/////Update submissions Table
$sqlupdateSm="update ".$prefix."submission set is_sent='0',status='$newStatus', CompletenessCheck='Rejected' where id='$protocol_idwe'";
$mysqli->query($sqlupdateSm);

$sqlupdateSm_protocol="update ".$prefix."protocol set revised_in=now(),Comments_at_Completion='Yes',Date_Comments_sent_to_Researcher=now() where id='$protocol_idwe' and owner_id='$owner_id'";
$mysqli->query($sqlupdateSm_protocol);

$sqlupdateSm2="update ".$prefix."submission_stages set status='new' where protocol_id='$protocol_idwe' and owner_id='$owner_id'";
$mysqli->query($sqlupdateSm2);


$sqlupdateSm2sections="update ".$prefix."updated_sections set status='submitted' where protocol_id='$protocol_idwe' and owner_id='$owner_id' and status='pending'";
$mysqli->query($sqlupdateSm2sections);

$sqlupdateSm2sectionsdos="update ".$prefix."submission_upload set new_old='old' where submission_id='$protocol_idwe' and user_id='$owner_id' and new_old='new'";
$mysqli->query($sqlupdateSm2sectionsdos);


//copy data from submissions table
//INSERT INTO ".$prefix."submission_archive(id, protocol_id, owner_id) SELECT id, protocol_id, owner_id FROM ".$prefix."submission WHERE WHERE id='$protocol_idwe'
//
/*$sqlupdateSm2Archieve1="INSERT INTO ".$prefix."submission_archive SELECT * FROM ".$prefix."submission WHERE id='$protocol_idwe'";
$mysqli->query($sqlupdateSm2Archieve1);   apvr_*/
///Insert into tidious process, the later failed because of duplicate entries.
require_once("viewlrcn/completeness_check_archieve_data.php");

require("viewlrcn/class.phpmailer.php");
require("viewlrcn/class.smtp.php"); 
require_once("viewlrcn/mail_template_completeness_check.php");
...
$mail->Subject = "Completeness Check (Incomplete) - $public_title";
$body="$allSentMessage
";
$mail->MsgHTML($body);
...
/////Update submissions Table
$sqlupdateSm="update ".$prefix."submission set is_sent='1',status='completeness check', CompletenessCheck='Approved' where id='$protocol_idwe'";
$mysqli->query($sqlupdateSm);

$sqlupdateSm2="update ".$prefix."submission_stages set status='completed' where protocol_id='$protocol_idwe' and owner_id='$owner_id'";
$mysqli->query($sqlupdateSm2);

$sqlupdateSm_protocol="update ".$prefix."protocol set revised_in=now(),protocol_review_date=now() where id='$protocol_idwe' and owner_id='$owner_id'";
$mysqli->query($sqlupdateSm_protocol);
/////Send Email
$sqlupdateSm2sections="update ".$prefix."updated_sections set status='submitted' where protocol_id='$protocol_idwe' and owner_id='$owner_id' and status='pending'";
$mysqli->query($sqlupdateSm2sections);
...
require("viewlrcn/class.phpmailer.php");
require("viewlrcn/class.smtp.php"); 
require_once("viewlrcn/mail_template_completeness_check_approved.php");
...
$mail->WordWrap = 50; // set word wrap to 50 characters
$mail->IsHTML(false); // set email format to HTML
$mail->Subject = "Completeness Check (Complete) - $public_title";
$body="$allSentMessage
";
$mail->MsgHTML($body);
...
echo $message="<p class='success'>Completeness Check for , '<b>$public_title</b> has been marked as <b>Complete</b>.'</p>";
echo '<meta http-equiv="REFRESH" content="2;url='.$base_url.'/main.php?option=dashboard">';
...
echo '<img src="images/loading_wait.gif">';
echo '<div class="spacer"></div>';
echo '<meta http-equiv="REFRESH" content="0;url='.$base_url.'/main.php?option=initialCommitteeReviews&id='.$protocol_idwe.'">';
...
<input id="go" type="button" value="Click to Change Review Type" onclick="window.open('<?php echo $base_url;?>changeofreview.php?id=<?php echo $id;?>','popUpWindow','height=700, width=1000, left=100, top=100, resizable=yes, scrollbars=yes, toolbar=yes, menubar=no, location=no, directories=no, status=yes');"  class="errorm4" >
...
<button class="accordion">Protocol Information, click to review <?php if($rprotocalSEdited['protocol_information']==1){?><span style="color:#F00;">Edited</span><?php }?></button>
  <div class="panel">
  <?php
...
<button class="accordion">Protocol Details, click to review <?php if($rprotocalSEdited['protocol_details']==1){?><span style="color:#F00;">Edited</span><?php }?></button>
  <div class="panel">
  <?php
...
<button class="accordion">Budget Information, click to review <?php if($rprotocalSEdited['budget']==1){?><span style="color:#F00;">Edited</span><?php }?></button>
  <div class="panel">
   <?php
...
<button class="accordion">Study Description, click to review <?php if($rprotocalSEdited['study_description']==1){?><span style="color:#F00;">Edited</span><?php }?></button>
  <div class="panel">
...
<button class="accordion">Study Population, click to review <?php if($rprotocalSEdited['study_population']==1){?><span style="color:#F00;">Edited</span><?php }?></button>
  <div class="panel">
...
<label class="form-control-label"><strong style="font-weight:bold;">Ethical Considerations:</strong> <?php echo $rstudymethodology['ethical_considerations'];?></label>
 ...
 <button class="accordion">Study Work Plan, click to review <?php if($rprotocalSEdited['study_work_plan']==1){?><span style="color:#F00;">Edited</span><?php }?></button>
  <div class="panel">
  <table class="table table-striped table-sm">
                        <thead>
                          <tr>
                        ...
                        <?php
//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y') AS created,DATE_FORMAT(`updated`,'%d/%m/%Y') AS updated FROM ".$prefix."submission_task where submission_id='$protocol_idwe' and owner_id='$owner_id' order by id desc LIMIT 0,10";//and conceptm_status='new'  %H:%s:%i
$result = $mysqli->query($sql);
while($rInvestigator=$result->fetch_array()){
	?>
                          <tr>
                            <td><a href="./files/uploads/<?php echo $rInvestigator['description'];?>" target="_blank"><?php echo $rInvestigator['description'];?></a></td>
  ...
  <button class="accordion">Attachments, click to review <?php if($rprotocalSEdited['attachments']==1){?><span style="color:#F00;">Edited</span><?php }?></button>
  <div class="panel">
  ...
  <table class="table table-striped table-sm">
                        ...
                        <?php
//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."submission_upload where user_id='$owner_id' and submission_id='$protocol_idwe' order by id desc LIMIT 0,20";//and conceptm_status='new' 
$result = $mysqli->query($sql);
while($rInvestigator=$result->fetch_array()){
...
$rUsers=$resultUser->fetch_array();
	?>
                          <tr>
                            ...
                            <td><?php echo $rInvestigator['created'];?> <?php if($rInvestigator['new_old']=='new'){?><span style="color:#F00;">Newly uploaded</span><?php }?></td>
...
<!--  <button class="accordion">History, click to review</button>
 ...
 <?php if($_SESSION['privillage']=='recreviewer' || $session_privillage=='recadmin' || $session_privillage=='rechairperson' || $session_privillage=='revicechairperson'){?>
 
  <button class="accordion">Comments, click to review </button>
  <div class="panel">
  
     <?php 
if($_POST['doComment']=='Save' and $_POST['message'] and $_SESSION['privillage']=='recreviewer'){
...
<?php }//to be restricted to reviewer?>
