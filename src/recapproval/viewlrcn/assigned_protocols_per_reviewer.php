...
<?php
$sql_suer = "select * from ".$prefix."user where asrmApplctID='$id'";
$result_user = $mysqli->query($sql_suer);
$sqUser_sr = $result_user->fetch_array();
?>

<div class="card-header d-flex align-items-center success">
                      <h3 class="h4">Protocols Assigned to <?php echo $sqUser_sr['name'];?></h3>
</div>
                    <div class="card-body">
...
$query = $mysqli->query("select COUNT(*) as num from apvr_submission,apvr_submission_review_sr where apvr_submission.protocol_id=apvr_submission_review_sr.protocol_id and apvr_submission_review_sr.asrmApplctID='$id' and is_sent='1' and recAffiliated_id='$recAffiliated_id' and (public_title like '%$searchprotocol%' OR code like '%$searchprotocol%' OR shtname like '%$searchprotocol%') and status='$status' order by apvr_submission.id desc");//and conceptm_status='new' 
}

if($_POST['status'] and !$_POST['searchprotocol']){	
$query = $mysqli->query("select COUNT(*) as num from apvr_submission,apvr_submission_review_sr where apvr_submission.protocol_id=apvr_submission_review_sr.protocol_id and apvr_submission_review_sr.asrmApplctID='$id' and is_sent='1' and recAffiliated_id='$recAffiliated_id' and  status='$status' order by apvr_submission.id desc");//and conceptm_status='new' 
}

if(!$_POST['status'] and $_POST['searchprotocol']){	
$query = $mysqli->query("select COUNT(*) as num from apvr_submission,apvr_submission_review_sr where apvr_submission.protocol_id=apvr_submission_review_sr.protocol_id and apvr_submission_review_sr.asrmApplctID='$id' and is_sent='1' and recAffiliated_id='$recAffiliated_id' and  (public_title like '%$searchprotocol%' OR code like '%$searchprotocol%' OR shtname like '%$searchprotocol%') order by apvr_submission.id desc");//and conceptm_status='new' 
...
if(!$_POST['doSearch']){
$query = $mysqli->query("select COUNT(*) as num from apvr_submission,apvr_submission_review_sr where apvr_submission.protocol_id=apvr_submission_review_sr.protocol_id and apvr_submission_review_sr.asrmApplctID='$id' and is_sent='1' and recAffiliated_id='$recAffiliated_id' order by apvr_submission.id desc");//and conceptm_status='new' 
}
...
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM apvr_submission,apvr_submission_review_sr where apvr_submission.protocol_id=apvr_submission_review_sr.protocol_id and apvr_submission_review_sr.asrmApplctID='$id' and is_sent='1'  and recAffiliated_id='$recAffiliated_id' and (public_title like '%$searchprotocol%' OR code like '%$searchprotocol%' OR shtname like '%$searchprotocol%') and status='$status' order by apvr_submission.id desc LIMIT 0, $limitm";//$start
}//'Pending','reviewed','Approved','Rejected'

if($_POST['status'] and !$_POST['searchprotocol']){							//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM apvr_submission,apvr_submission_review_sr where apvr_submission.protocol_id=apvr_submission_review_sr.protocol_id and apvr_submission_review_sr.asrmApplctID='$id' and is_sent='1'  and recAffiliated_id='$recAffiliated_id' and status='$status' order by apvr_submission.id desc LIMIT 0, $limitm";//and conceptm_status='new' 
}//'Pending','reviewed','Approved','Rejected'

if(!$_POST['status'] and $_POST['searchprotocol']){							//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM apvr_submission,apvr_submission_review_sr where apvr_submission.protocol_id=apvr_submission_review_sr.protocol_id and apvr_submission_review_sr.asrmApplctID='$id' and recAffiliated_id='$recAffiliated_id' and (public_title like '%$searchprotocol%' OR code like '%$searchprotocol%' OR shtname like '%$searchprotocol%') order by apvr_submission.id desc LIMIT 0, $limitm";//and conceptm_status='new' 
}//'Pending','reviewed','Approved','Rejected'
...
$sqprotocolReviewr = $resultReviewr->fetch_array();
	?>
                          <tr>
                            <th scope="row"><?php echo $sqprotocol['code'];?> </th>
...
<?php echo "<b style='color:#796AEE;'>".$rProjects['recstatus']."</b>";  ?>
...
<a href="print.php?pr=<?php echo $rProjects['id'];?>" target="_blank"><span class="label label-sec3">+ Print Submission</span></a>
                            </td>
                          </tr>
   <?php }///////////end function ?>                 
                        </tbody>
