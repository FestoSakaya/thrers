...
<?php
if($_POST['doSubmitDeviations']=='Submit Details'){
...
$sqlprotocalSubSel="SELECT * FROM ".$prefix."deviations where protocol_id='$project_id' and owner_id='$asrmApplctID' and PDDateofoccurrence='$PDDateofoccurrence' and PDDescriptionofdeviation='$PDDescriptionofdeviation' and Rootcauseofdeviation='$Rootcauseofdeviation' and Correctiveactiontaken='$Correctiveactiontaken'";
...
values('$asrmApplctID','$project_id','$recAffiliated_id','$PDDateofoccurrence','$PDDescriptionofdeviation','$Rootcauseofdeviation','$Correctiveactiontaken','','$PVDateofoccurrence','$PVDescriptionofdeviation','$parta','$partaOther','$partb','$Rootcauseofviolation_b','$Correctiveaction_b','$Measurestomitigateviolation_b',now(),'Pending')";
$mysqli->query($sqlA2);
...
if($totalstudy){
	$sqlA2="update ".$prefix."deviations set  `PDDateofoccurrence`='$PDDateofoccurrence',`PDDescriptionofdeviation`,`Rootcauseofdeviation`='$PDDescriptionofdeviation',`Correctiveactiontaken`='$Correctiveactiontaken',`Measurestomitigatedeviation`='$Measurestomitigatedeviation',`PVDateofoccurrence`='$PVDateofoccurrence',`PVDescriptionofdeviation`='$PVDescriptionofdeviation',`parta`='$parta',`partaOther`='$partaOther',`partb`='$partb',`Rootcauseofviolation_b`='$Rootcauseofviolation_b',`Correctiveaction_b`='$Correctiveaction_b',`Measurestomitigateviolation_b`='$Measurestomitigateviolation_b' where owner_id='$asrmApplctID' and protocol_id='$project_id'";
$mysqli->query($sqlA2);
$message='<p class="success">Dear '.$session_fullname.', details have been submitted, click finish button below to complete your submission</p>';	
}
...
$wmObjectives="select * from ".$prefix."saes_measures_mitigate_dev where  owner_id='$asrmApplctID' and protocol_id='$project_id' and Measurestomitigatedeviation='$institutioncode'";
...
$Insert_QR2="insert into ".$prefix."saes_measures_mitigate_dev (`protocol_id`,`owner_id`,`sae_id`,`Measurestomitigatedeviation`,`created`) values ('$project_id','$asrmApplctID','$sae_id','$institutioncode',now())";
...
$wmObjectivesm="select * from ".$prefix."saes_measures_mitigate_dev_b where  owner_id='$asrmApplctID' and protocol_id='$project_id' and Measurestomitigatedeviation='$institutioncodem'";
$cmdwbObjectivesm = $mysqli->query($wmObjectivesm);
$totalObjectivesm = $cmdwbObjectivesm->num_rows;

if(strlen($institutioncodem)>=5 and !$totalObjectivesm){
	
$Insert_QR2m="insert into ".$prefix."saes_measures_mitigate_dev_b (`protocol_id`,`owner_id`,`sae_id`,`Measurestomitigatedeviation`,`created`) values ('$project_id','$asrmApplctID','$sae_id','$institutioncodem',now())";
...
$email=$recUser['email'];
	//FInish submission
$sqlA2Protocol1="update ".$prefix."deviations  set `status`='Submitted',is_sent='1' where `owner_id`='$asrmApplctID' and deviationID='$id'";
...
$mail->Host = "smtp.gmail.com"; // specify SMTP server//nemesis.eahd.or.ug mailhost02.cfi.co.ug
$mail->SMTPAuth = true; // turn on SMTP authentication
$mail->SMTPSecure = 'ssl';

$mail->Username = "uncstuncstapps@gmail.com"; // SMTP username -- CHANGE --
$mail->Password = "lpupvbvillxraaey"; // SMTP password -- CHANGE --
$mail->From = "uncstuncstapps@gmail.com"; //From Address -- CHANGE -
/////////////////////////////Begin Mail Body
...
$sqlstudy="SELECT * FROM ".$prefix."deviations where `owner_id`='$asrmApplctID' and status='Pending' order by deviationID desc limit 0,1";
$Querystudy = $mysqli->query($sqlstudy);
...
function insRow()
{
  ...
  if($_GET['comobje']=='del'){
$qRDel="delete from ".$prefix."saes_measures_mitigate_dev where owner_id='$asrmApplctID' and protocol_id='$protocol_id' and id='$id'";
$mysqli->query($qRDel);
}

$qRPersoneld="select * from ".$prefix."saes_measures_mitigate_dev  where owner_id='$asrmApplctID' and protocol_id='$protocol_id'";
...
<?php echo $rowRows['Measurestomitigatedeviation'];?> <a href="main.php?option=Deviations&id=<?php echo $rowRows['id'];?>&comobje=del" style="color:#F00; font-weight:bold;" onclick="return confirm('Are you sure you want to delete this item?');">Delete</a></label><br />
<?php
}
 ...
 <?php }//end totas?>                        
                        ...
                        <div class="line"></div>
                         ...
                         <div class="form-group row success">
                          ...
                          <label class="col-sm-4 form-control-label">1.	Date of occurrence:</label>
                          <input type="date" name="PVDateofoccurrence" id="PVDateofoccurrence" class="form-control  required" value="<?php echo $rstudy['PVDateofoccurrence'];?>">
                        </div>
                        <div class="line"></div>
                          ...
                          <textarea name="PVDescriptionofdeviation" id="MyTextBox" cols="" rows="5" class="form-control  required"><?php echo $rstudy['PVDescriptionofdeviation'];?></textarea>       
                  <p id="countermm">Characters limit: <span  id="countercol">250 words</span></p>
...
<label class="col-sm-4 form-control-label">3. Root cause of violation </label>
  ...
  <textarea name="Rootcauseofviolation_b" id="MyTextBox7" cols="" rows="5" class="form-control  required"><?php echo $rstudy['Rootcauseofviolation_b'];?></textarea>       
                  <p id="countermm">Characters limit: <span  id="countercol">250 words</span></p>
...
<textarea name="Correctiveaction_b" id="MyTextBox7" cols="" rows="5" class="form-control  required"><?php echo $rstudy['Correctiveaction_b'];?></textarea>       
                  <p id="countermm">Characters limit: <span  id="countercol">250 words</span></p>
...
$qRDel2="delete from ".$prefix."saes_measures_mitigate_dev_b where owner_id='$asrmApplctID' and protocol_id='$protocol_id' and id='$id'";
$mysqli->query($qRDel2);
}

$qRPersoneld2="select * from ".$prefix."saes_measures_mitigate_dev_b  where owner_id='$asrmApplctID' and protocol_id='$protocol_id'";
...
<input type="text" name="Measurestomitigateviolation_b[]" id="Measurestomitigateviolation_b" tabindex="5"  class="form-control required" minlength="8" style="
    width:700px!important;
    padding: .5rem .75rem;
    font-size: 0.9rem;
    line-height: 1.25;
...
{ ///Display data for education history
	?>  <label class="form-control-label">
<?php echo $rowRows2['Measurestomitigatedeviation'];?> <a href="main.php?option=Deviations&id=<?php echo $rowRows2['id'];?>&comobjem=del" style="color:#F00; font-weight:bold;" onclick="return confirm('Are you sure you want to delete this item?');">Delete</a></label><br />
<?php
}
...
?> 
 ...
 <?php }//end totas?> 
                    ...
                    <input name="doSubmitDeviations" type="submit"  class="btn btn-primary" value="Submit Details"/>
                    
                    
                    <?php if($totalstudy){?>					
					<input name="doSaveFinish" type="button"  class="btn-secondary" value="Make Final Submission" style="float:right; margin-top:5px; "  onClick="window.location.href='<?php echo $base_url;?>main.php?option=FinalSubmitDeviations&id=<?php echo $rstudy['protocol_id'];?>'"/>
