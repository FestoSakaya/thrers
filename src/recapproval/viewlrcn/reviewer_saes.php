...
<div class="card-header d-flex align-items-center">
                      <h3 class="h4">SAEs for Review</h3>
</div>
                    <div class="card-body">
...
$adjacents = 3;
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submission_review_sr where reviewer_id='$asrmApplctID' and reviewFor='SAEs' order by id desc");//and conceptm_status='new'
...
$sql = "select * FROM ".$prefix."submission_review_sr where  reviewer_id='$asrmApplctID'  and reviewFor='SAEs' order by id desc LIMIT $start, $limitm";//and conceptm_status='new'
...
$sqUserddRREC2 = $resultSSSREC2->fetch_array();
  ...
  <?php if($sqReview['is_clinical_trial']==0){?><span class="label label-primary"><?php echo $sqUserddRREC2['rstug_categoryName'];?></span> <?php }?>                          </td>
   <td><?php echo $sqUserddRREC['name'];?></td>
                            ...
                            <td>
<?php if($rProjects['recstatus']=='Pending' || $rProjects['recstatus']=='new' and $rProjects['conflictofInterest']=='no'){?><a href="./main.php?option=ReviewerInitialSAEs&id=<?php echo $sqReview['id'];?>" style=" color:#ffffff;" class="label label-sec4">Review Submission</a><br />
