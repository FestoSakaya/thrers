...
if($_POST['doAmmendments']=='Save & Continue' and $_POST['recAffiliated_id'] and $_POST['Changes'] and $sessionasrmApplctID and $_POST['protocol_title'] and $_SESSION['asrmApplctID'] and $_POST['ManualrefNo']){
...
$sqlstudyPrject="SELECT * FROM ".$prefix."submission where `owner_id`='$sessionasrmApplctID' order by id desc limit 0,1";
$QueryPrject = $mysqli->query($sqlstudyPrject);
$totalsPrject = $QueryPrject->num_rows;
$rstudymPrject = $QueryPrject->fetch_array();
//$public_title=$rstudymPrject['public_title'];
$refNo=$rstudymPrject['code'];

if($_FILES['Attachnewconsentform']['name'] and $id){
...
$sqlAAttachnewconsentform="update ".$prefix."ammendments  set `Attachnewconsentform`='$fileattachment1' where `owner_id`='$sessionasrmApplctID' and ammendType='manual' and is_sent='0' and id='$id'";
$mysqli->query($sqlAAttachnewconsentform);
}

if($_FILES['Attachnewtool']['name'] and $id){
...
$sqlAAttachnewtool="update ".$prefix."ammendments  set `Attachnewtool`='$fileattachment2' where `owner_id`='$sessionasrmApplctID' and ammendType='manual' and is_sent='0' and id='$id'";
$mysqli->query($sqlAAttachnewtool);
}

if($_FILES['Attachnewprotocol']['name'] and $id){
...
$sqlAAttachnewprotocol="update ".$prefix."ammendments  set `Attachnewprotocol`='$fileattachment3' where `owner_id`='$sessionasrmApplctID' and ammendType='manual' and is_sent='0' and id='$id'";
...
$sqlprotocalSubSelCAllmm="SELECT * FROM ".$prefix."ammendments where owner_id='$sessionasrmApplctID' and ammendType='manual' and is_sent='0' order by id desc";
...
if($record_id){
echo '<meta http-equiv="REFRESH" content="0;url='.$base_url.'/main.php?option=ManualAmmendments&id='.$record_id.'">';
}
}

if($totalstudy3  and $id){
$sqlA22="update ".$prefix."ammendments  set `listchanges`='$Changes',`ReasonforAmendment`='$ReasonforAmendment',`changestostudyDistricts`='$changestostudyDistricts',`ChangestoConsentForm`='$ChangestoConsentForm',`ChangestodataCollectionTool`='$ChangestodataCollectionTool',`ChangestoProtocol`='$ChangestoProtocol',`protocol_title`='$public_title',`refNo`='$ManualrefNo' where `owner_id`='$sessionasrmApplctID'  and id='$id'";
$mysqli->query($sqlA22);
$message='<div class="success">Changes have saved</div>';

echo '<img src="images/loading_wait.gif">';
echo '<div class="spacer"></div>';
echo '<meta http-equiv="REFRESH" content="0;url='.$base_url.'/main.php?option=ManualAmmendmentsSec&id='.$id.'">';
}
...
<?php 
$wmRenewals="select *,DATE_FORMAT(`created`,'%Y-%m-%d') AS created from ".$prefix."ammendments  where `owner_id`='$asrmApplctID' and is_sent='0'  and ammendType='manual' and id='$id' order by id desc limit 0,1";
...
<?php $owner_id=$_SESSION['asrmApplctID'];?>
  ...
  <textarea name="protocol_title" id="protocol_title" cols="" rows="5" class="form-control" required><?php echo $rRenewals['protocol_title'];?></textarea>
 ...
 <label class="col-sm-12 form-control-label">Reference Number: <br /></label>
  ...
  <textarea name="ManualrefNo" id="ManualrefNo" cols="" rows="2" class="form-control" required><?php echo $rRenewals['refNo'];?></textarea>
...
<div style="margin-left:15px;">Attach new consent form<br />
<input name="Attachnewconsentform" type="file" id="Attachnewconsentform"/>
</div>  
...
<div style="margin-left:15px;">Attach new protocol<br />
<input name="Attachnewprotocol" type="file" id="Attachnewprotocol" class="required" required/>
</div>
                    ...
                    <input name="doAmmendments" type="submit"  class="btn btn-primary" value="Save & Continue"/>
