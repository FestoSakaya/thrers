...
<li><a href="./main.php?option=ManualNotifications&id=<?php echo $id;?>&rec=<?php echo $rec;?>"  style="background:green; color:#FFF;">Apply for Notifications</a></li>
...
if($_POST['doFilesUpload']=='Save' and $rec and $id){
...
$sqlprotocalSubSel3="SELECT * FROM ".$prefix."notifications_attachments where notification_id='$id' and owner_id='$sessionasrmApplctID' and fileAttachment='$fileattachment'";
$QprotocalSub2Sel3 = $mysqli->query($sqlprotocalSubSel3);
$totalstudy3 = $QprotocalSub2Sel3->num_rows;

if(!$totalstudy3){
$sqlA22="insert into ".$prefix."notifications_attachments (`owner_id`,`protocol_id`,`recAffiliated_id`,`fileAttachment`,`created`,`notification_id`) 

values('$sessionasrmApplctID','$project_id','$rec','$fileattachment',now(),'$id')";
$mysqli->query($sqlA22);
$message='<div class="success">Changes have saved</div>';
}

if($totalstudy3){	
$sqlA2="update ".$prefix."notifications_attachments set `fileAttachment`='$fileattachment' where owner_id='$sessionasrmApplctID' and notification_id='$id' and fileAttachment='$fileattachment'";
//$mysqli->query($sqlA2);
//$message='<div class="success">Changes have saved</div>';
}
...
if($category=='FinalManualnotifications' and $id){

$project_id=$mysqli->real_escape_string($id);
	////Get Protocol ID
$sqlstudy="SELECT * FROM ".$prefix."notifications where id='$id' order by id desc limit 0,1";
...
$sqlA2Protocol1="update ".$prefix."notifications  set `status`='Submitted' where `owner_id`='$sessionasrmApplctID' and id='$id'";
...
$sqlstudy="SELECT *,DATE_FORMAT(`created`,'%Y-%m-%d') AS created FROM ".$prefix."notifications_attachments where `owner_id`='$sessionasrmApplctID'  and notification_id='$id' order by id desc";
$Querystudy = $mysqli->query($sqlstudy);
...
$wmSubmissions="select * from ".$prefix."notifications where  `id`='$id'";
...
<?php
$wmRenewals22="select * from ".$prefix."notifications where `owner_id`='$sessionasrmApplctID'  and id='$id' and status='Pending' order by id desc";
...
<?php if($totalstudy){?>					
					<input name="doSaveFinish" type="button"  class="btn-secondary" value="Make Final Submission" style="float:right; margin-top:5px; "  onClick="window.location.href='<?php echo $base_url;?>main.php?option=FinalManualnotifications&id=<?php echo $rRenewals22['id'];?>&rec=<?php echo $rec;?>'"/>
