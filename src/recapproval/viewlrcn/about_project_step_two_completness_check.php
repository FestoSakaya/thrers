...
<?php
$sqlstudym="SELECT * FROM ".$prefix."submission where `owner_id`='$asrmApplctID' and id='$id' and protocol_id='$protocol_id' order by id desc limit 0,1";
                ...
                <div class="row bg-white has-shadow">
                  <div class="left-col col-lg-6 d-flex align-items-center justify-content-between">
                    <div class="project-title d-flex align-items-center">
                     <?php if($sqUserdd['profile']){?> <div class="image has-shadow"><img src="files/
profile/<?php echo $sqUserdd['profile'];?>" alt="..." class="img-fluid"></div><?php }?>
                      <div class="text">
...
$institutioncode=$_POST['objectives'][$i];
$institutioncode2.=$_POST['objectives'][$i];
$objectivetype=$mysqli->real_escape_string($_POST['objectivetype'][$i]);



$wmObjectives="select * from ".$prefix."other_objectives where  owner_id='$sasrmApplctID' and protocol_id='$id' and objective='$institutioncode'";
...
$Insert_QR2="insert into ".$prefix."other_objectives (`owner_id`,`protocol_id`,`objective`,`created`,`objectivetype`) values ('$sasrmApplctID','$id','$institutioncode',now(),'$objectivetype')";
...
$sqlA2Protocol="update ".$prefix."submission  set `abstract`='$abstract',`keywords`='$keywords',`introduction`='$introduction',`justification`='$justification',`goals`='$goals',`objective_1`='$institutioncode2',`objective_2`='$objective_2' where id='$id' and owner_id='$sasrmApplctID'";
...
$r= $cmdwb->fetch_array();
if($totalStages){
$sqlASubmissionStages="update ".$prefix."submission_stages  set `protocol_details`='1' where `owner_id`='$sasrmApplctID' and `protocol_id`='$id'";
...
logaction("$session_fullname updated protocol");
	...
	//update edited table...
$sqlURevisions="SELECT * FROM ".$prefix."updated_sections where `owner_id`='$asrmApplctID' and protocol_id='$protocol_id' and status='pending' order by id desc limit 0,1";
...
values('$asrmApplctID','$protocol_id','','','1','','','','','','','','','',now(),'pending')";
$mysqli->query($sqlAREvisedSections);
}
if($totalUsersRevions){

$sqlAREvisedSections_update="update ".$prefix."updated_sections  set `protocol_details`='1' where owner_id='$asrmApplctID' and protocol_id='$protocol_id' and status='pending'";
...
echo '<meta http-equiv="REFRESH" content="1;url='.$base_url.'/main.php?option=submissionThirdUp&id='.$id.'">';

}//end post 


 $did=$mysqli->real_escape_string($_GET['did']);
  if($_GET['comobje']=='del' and $id and $_GET['did']){
...
<h5>Main Objectives</h5>
   ...
   <?php
  $count1=0;
$qRPersoneld="select * from ".$prefix."other_objectives  where owner_id='$asrmApplctID' and protocol_id='$protocol_id' and objectivetype='Main Objective' order by objectivetype asc";
$RPersoneld=$mysqli->query($qRPersoneld);
...
<h5>Specific Objectives</h5>
  ...
  <?php
   $count2=0;
$qRPersoneld4="select * from ".$prefix."other_objectives  where owner_id='$asrmApplctID' and protocol_id='$protocol_id' and objectivetype='Specific Objective' order by objectivetype asc";
...
<label class="form-control-label">
<?php echo $count2.'. '.$rowRows4['objective'];?> <a href="main.php?option=submissionSecondUp&id=<?php echo $id;?>&did=<?php echo $rowRows4['id'];?>&comobje=del" style="color:#F00; font-weight:bold;" onclick="return confirm('Are you sure you want to delete this item?');">Delete</a></label><br />
<?php
}
