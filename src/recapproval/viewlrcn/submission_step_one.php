...
$mysqli->query($sqlA2Protocol);	
$sqlANov="update ".$prefix."submission_stages  set `protocol_information`='1' where `owner_id`='$sasrmApplctID' and `protocol_id`='$id'";
$mysqli->query($sqlANov);
...
<li><a href="<?php echo $base_url;?>recapprova./main.php?option=submission" rel="#default" class="selected" <?php if($rsSubStages['protocol_information']==1 and $rsSubStages['protocol_team']==1 and $totalSub_Stages){?> style="background:green; color:#FFF;" <?php }?>>Protocol Information</a></li>
...
$completionyeareduc=$mysqli->real_escape_string($_POST['completionyeareduc'][$i]);
...
$sqlInvestigatorsT="SELECT * FROM ".$prefix."team where `owner_id`='$asrmApplctID_session' and email='$email' order by id desc limit 0,1";//and protocol_id='$id' 
...
////////////////////////////////////////////////////////////////////////
$sqlInvestigatorsT2="SELECT * FROM ".$prefix."team where `owner_id`='$asrmApplctID_session' and protocol_id='$id' order by id desc limit 0,1";//and protocol_id='$id' 
$QueryInvestigatorsTr2 = $mysqli->query($sqlInvestigatorsT2);//and protocol_id='$id' 
...
$new_rstug_educn_year=$rTeamHistory['rstug_educn_year'];
$new_completionyear=$rTeamHistory['completionyear'];
$new_rstug_educn_period=$rTeamHistory['rstug_educn_period'];
...
$new_completionyear_emp=$rTeamEmploymentD['completionyear'];
...
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."team where owner_id='$asrmApplctID' and  status='new' and protocol_id='$id' order by id desc LIMIT 0,100";//and conceptm_status='new' 
...
<input id="go" type="button" value="Click here to Add New" onclick="window.open('$base_urlbiodata.php?id=<?php echo $rInvestigator['id'];?>','popUpWindow','height=700, width=1000, left=100, top=100, resizable=yes, scrollbars=yes, toolbar=yes, menubar=no, location=no, directories=no, status=yes');"  class="errorm4" ><?php }?>
