...
$counTno=0;
$queryDocuments="select * from apvr_ammendments_documents where amendment_id='$renewal_id' and includedon_approval='Yes' order by id desc";
$cmdwDocuments=$mysqli->query($queryDocuments);
while($rSwDocuments=$cmdwDocuments->fetch_array()){
$counTno++;
if($rSwDocuments['atype']=='otherattachment'){$otherName=$rSwDocuments['otherattachment'];}
if($rSwDocuments['atype']!='otherattachment'){$otherName=$rSwDocuments['atype'];}


$aDate=$rSwDocuments['aDate'];
$aLanguage=$rSwDocuments['aLanguage'];
$aVersion=$rSwDocuments['aVersion'];

$docummentsincluded.="<tr>
    <th width='6%' valign='top'><p><strong>$counTno</strong></p></th>
    <th width='60%' valign='top'>$otherName</th>
	<th width='60%' valign='top'>$aLanguage</th>
    <th width='20%' valign='top'>$aVersion</th>
	<th width='20%' valign='top'>$aDate</th>
  </tr>";
  $lastno=$counTno;
}
...
<p>&nbsp;</p><p>&nbsp;</p><p>&nbsp;</p><p>&nbsp;</p>
<p>The following is the list of all documents reviewed in this application by <b>$RECName</b>:
</p>
