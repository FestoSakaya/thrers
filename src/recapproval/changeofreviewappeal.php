...
?>
         <h2 class="no-margin-bottom">Appeal for Change Review Type</h2>
       ...
       <?php
//doSaveFive
if($_POST['doSaveAppeal'] and $_POST['asrmApplctID'] and $id and $_POST['reasonsforappeal']){
	...
	$reasonsforappeal=$mysqli->real_escape_string($_POST['reasonsforappeal']);
...
require_once("viewlrcn/mail_template_change_review_appeal.php");
...
$mail->Subject = "Appeal for Change Review Type - $public_title";
$body="$allSentMessage
";
$mail->MsgHTML($body);
...
$sqlstudymUpdate="update ".$prefix."protocol set changeofreview='appeal',reasonsforappeal='$reasonsforappeal' where id='$id'";
$mysqli->query($sqlstudymUpdate);
logaction("$session_fullname made an appeal for change of review type of $public_title");
...
<option value="Exempt" <?php if($rstudym['type_of_review']=='Exempt'){?>selected="selected"<?php }?>>Exempt</option>
                         ...
                         <div class="form-group row success">
                          <label class="col-sm-7 form-control-label">Provide reasons for appeal: <span class="error">*</span></label>
                          <textarea name="reasonsforappeal" id="mawanda1hh" cols="" rows="5" class="form-control  required"><?php if($rstudym['reasonsforappeal']){echo $rstudym['reasonsforappeal'];}else{$_POST['reasonsforappeal'];}?></textarea>
                        </div>
 ...
 <div id="conflictdiv"></div>  
                    ...
                    <input name="doSaveAppeal" type="submit"  class="btn btn-primary" value="Appeal"/>
