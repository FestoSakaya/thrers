<?php if($session_privillage=='administrator' || $session_privillage=='recitadmin'){?><button id="myBtn">Add IT Administrator for REC </button>
<form action="" method="post" name="regForm" id="regForm" autocomplte="off">
<table width="100%" border="0" style="margin-top:15px;">
  <tr>
    <td width="59%">Find IT REC Admins:<br />
...
Status:<br />
                                ...
                                <option value="S" selected="selected">REC IT Admin</option>
          </select>
...
values('$country_id',now(),now(),'$email','$password','$username','1','$name','$institution','','0','recitadmin','','$recAffiliated_idTo','','$activ_code','$firstname','','$lastname')";
$mysqli->query($sqlA2);
...
$mail->addCc('uncstuncstapps@gmail.com',$name);
...
$mail->Subject = "REC IT Administrator Account";
$body="
Dear $name !<br><br>
An account has been created for you as REC IT Administrator. You will use these logins whenever to log into the system. Below are your login details;<br><br>
...
<div class="card-header d-flex align-items-center">
                      <h3 class="h4">REC IT Administrators </h3>
</div>
                    <div class="card-body">
...
$adjacents = 3;
if($_POST['searchname']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."user where name like '%$name%' and recAffiliated_id='$id' and privillage='recitadmin' order by asrmApplctID desc");//and conceptm_status='new'
}
if(!$_POST['searchname'] and $session_privillage=='administrator'){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."user where  privillage='recitadmin' order by asrmApplctID desc");//and conceptm_status='new'
}

if(!$_POST['searchname'] and $session_privillage!='administrator'){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."user where recAffiliated_id='$id' and privillage='recitadmin' order by asrmApplctID desc");//and conceptm_status='new'
}
...
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."user where name like '%$name%' and recAffiliated_id='$id' and privillage='recitadmin' order by asrmApplctID desc LIMIT $start, $limitm";//and conceptm_status='new'
}
if(!$_POST['searchname'] and $session_privillage!='administrator'){//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."user where recAffiliated_id='$id' and privillage='recitadmin' order by asrmApplctID desc LIMIT $start, $limitm";//and conceptm_status='new'
}

if(!$_POST['searchname'] and $session_privillage=='administrator'){//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."user where  privillage='recitadmin' order by asrmApplctID desc LIMIT $start, $limitm";//and conceptm_status='new'
}
...
<div id="myModal" class="modal" style="margin-top:53px;">
  ...
  <div class="modal-content">
    <div class="modal-header">
      ...
      <h3><strong>Add new IT Administrator</strong></h3>
    </div>
...
<div class="col-sm-10" style="color:#F00;">
