<?php if($session_privillage=='recadmin' || $session_privillage=='rechairperson' || $session_privillage=='revicechairperson' || $session_privillage=='recitadmin' || $session_privillage=='membercommittee' || $session_privillage=='recreviewer' || $session_privillage=='administrator'){?>
...
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submission where is_sent='1' and (public_title like '%$searchprotocol%' OR code like '%$searchprotocol%' OR shtname like '%$searchprotocol%' OR code like '%$searchprotocol%')  order by id desc");//and conceptm_status='new' 
}
...
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."submission where is_sent='1' and (public_title like '%$searchprotocol%' OR objective_1 like '%$searchprotocol%' OR introduction like '%$searchprotocol%'  OR code like '%$searchprotocol%') order by id desc LIMIT $start, $limitm";//and conceptm_status='new' 
...
if(!$_POST['status'] and $_POST['searchprotocol']){							//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."submission where is_sent='1' and (public_title like '%$searchprotocol%' OR objective_1 like '%$searchprotocol%' OR introduction like '%$searchprotocol%'  OR code like '%$searchprotocol%') order by id desc LIMIT $start, $limitm";//and conceptm_status='new' 
}//'Pending','reviewed','Approved','Rejected' public_title like '%$searchprotocol%' OR code like '%$searchprotocol%'
...
<?php //echo $rProjects['introduction'];//echo $text = preg_replace('#'. preg_quote($searchprotocol) .'#i', '<span class="hlw">\\0</span>', $text);?>	

<a href="<?php echo $base_url;?>recapprova./main.php?option=allSubmittedProtocols"><?php echo $rProjects['public_title'];?> - <?php echo $rProjects['code'];?></a></h3>

<?php
$qRPersoneld="select * from ".$prefix."other_objectives  where owner_id='$owner_id' and protocol_id='$main_submission_id'";
$RPersoneld=$mysqli->query($qRPersoneld);
while($rowRows = $RPersoneld->fetch_array()){
 echo $rowRows['objective'].'<br>';
}?>
<?php if($shtname){?><br><strong>By: <?php echo $shtname;?></strong><?php }?>
</td>
