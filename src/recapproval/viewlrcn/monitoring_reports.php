...
function getExtension($str) {
   ...
   $manualTitle=$mysqli->real_escape_string($_POST['manualTitle']);
...
$sqlstudy="SELECT * FROM ".$prefix."monitoring_reports where `recAffiliated_id`='$recAffiliated_id' and DocumentName='$DocumentName'  and Attachment='$Attachment2'";//
$Querystudy = $mysqli->query($sqlstudy);
$totalstudy = $Querystudy->num_rows;
if(!$totalstudy){
$sqlA2="insert into ".$prefix."monitoring_reports (`owner_id`,`protocol_id`,`recAffiliated_id`,`DocumentName`,`Version`,`Description`,`Attachment`,`docDate`,`manualTitle`) 

values('$asrmApplctID','$protocol_id','$recAffiliated_id','$DocumentName','$Version','$Description','$Attachment2',now(),'$manualTitle')";
$mysqli->query($sqlA2);
...
if (!unlink($file))
 ...
 $message="<span class=error2>Monitoring Reports has been successfully removed.</span>"; 
}
...
<table width="100%" border="0" style="margin-top:15px;">
  <tr>
    <td width="90%">Find Monitoring Reports:<br />
...
<button id="myBtn">Add New Monitoring Report </button> 

<div class="card-header d-flex align-items-center">
                      <h3 class="h4">Monitoring Reports</h3>
</div>
                    <div class="card-body">
...
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."monitoring_reports where recAffiliated_id='$recAffiliated_id' and (DocumentName like '%$searchprotocol%' OR Version like '%$searchprotocol%' OR Attachment like '%$searchprotocol%')  order by id desc");//and conceptm_status='new' 
}

if(!$_POST['searchprotocol']){	
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."monitoring_reports where recAffiliated_id='$recAffiliated_id' and  status='$status' order by id desc");//and conceptm_status='new' 
}
...
if(!$_POST['doSearch']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."monitoring_reports where recAffiliated_id='$recAffiliated_id' order by id desc");//and conceptm_status='new' 
}
...
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."monitoring_reports where recAffiliated_id='$recAffiliated_id' and (DocumentName like '%$searchprotocol%' OR Version like '%$searchprotocol%' OR Attachment like '%$searchprotocol%')  order by id desc LIMIT $start, $limitm";//and conceptm_status='new' 
}//'Pending','reviewed','Approved','Rejected'

if(!$_POST['searchprotocol']){							//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."monitoring_reports where recAffiliated_id='$recAffiliated_id'  order by id desc LIMIT $start, $limitm";//and conceptm_status='new' 
}//'Pending','reviewed','Approved','Rejected'
...
if(!$_POST['doSearch']){
$sql = "select *,DATE_FORMAT(`docDate`,'%d/%m/%Y %H:%s:%i') AS docDatem FROM ".$prefix."monitoring_reports where recAffiliated_id='$recAffiliated_id' order by id desc LIMIT $start, $limitm";//and conceptm_status='new'
}
...
$sqprotocol = $resultprotocol->fetch_array();
	...
	?>
                          <tr>
                            ...
                            <td><h3 class="h4"><?php if($sqprotocol['manualTitle']){echo $sqprotocol['manualTitle'];}else{echo $sqprotocol['public_title'];}?></h3></td>
                            <td><?php echo $rProjects['DocumentName'];?></td>
 ...
 <a href="main.php?option=MonitoringReports&comnme=delete&id=<?php echo $rProjects['id'];?>" style="color:#F00; font-weight:bold;" onclick="return confirm('Are you sure you want to delete this Attendence?');">Remove File</a>
                            </td>
                          </tr>
   <?php }///////////end function ?>                 
                        </tbody>
...
<label class="col-sm-10 form-control-label">Select Protocol you are submitting to: </label>
<div class="col-sm-10">

<select name="protocol_id" id="protocol_id" class="form-control">
<option value="">Please Select Protocol</option>
<?php
$sqlSubmission = "select * FROM ".$prefix."submission where status='Approved' and recAffiliated_id='$recAffiliated_id' order by id desc"; 
...
<label class="col-sm-8 form-control-label"><strong style="color:#F00;">Protocol Title</strong>, for manual protocols not submitted through the system: <span class="error">*</span></label>
<div class="col-sm-10">
<textarea name="manualTitle" id="manualTitle" cols="" rows="" class="form-control"></textarea>
</div>
</div> 
