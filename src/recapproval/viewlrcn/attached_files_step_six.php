...
$sqlstudy="SELECT * FROM ".$prefix."submission_upload where `user_id`='$asrmApplctID' and submission_id='$id' and upload_type_id='$upload_type_id' and filename='$attachethicalapproval2' order by id desc";// and filename='$attachethicalapproval2'
$Querystudy = $mysqli->query($sqlstudy);
$totalstudy = $Querystudy->num_rows;
if(!$totalstudy){
$sqlA2="insert into ".$prefix."submission_upload (`user_id`,`submission_id`,`upload_type_id`,`created`,`updated`,`filename`,`filepath`,`submission_number`,`is_monitoring_action`,`Version`,`Language`,`DateofProposal`,`othername`) 

values('$asrmApplctID','$id','$upload_type_id',now(),now(),'$attachethicalapproval2','','','','$Version','$Language','$DateofProposal','$othername')";
$mysqli->query($sqlA2);


$sqlEwdd = "select * FROM ".$prefix."submission_upload where user_id='$asrmApplctID' and submission_id='$id'";
...
echo '<meta http-equiv="REFRESH" content="0;url='.$base_url.'/main.php?option=submissionFinish&id='.$id.'">';
...
<h3>Attachments:</h3>
 ...
 <table class="table table-striped table-sm" id="customers">
	...
	if($category=='submissionSixDel' and $did){

	$sqlA2Protocol2="delete from ".$prefix."submission_upload where user_id='$asrmApplctID' and submission_id='$id' and id='$did'";
	$mysqli->query($sqlA2Protocol2);
	}
						
						
//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%Y-%m-%d') AS created FROM ".$prefix."submission_upload where user_id='$asrmApplctID' and submission_id='$id' order by id desc LIMIT 0,200";//and conceptm_status='new' 
...
<td><a href="./main.php?option=submissionSixDel&did=<?php echo $rInvestigator['id'];?>&id=<?php echo $id;?>&del=delete" style="color:#F00; font-weight:bold;" onclick="return confirm('Are you sure you want to delete this item?');">Delete</a></td>
                          </tr>
   <?php }///////////end function ?>                 
                        </tbody>
                      </table>
...
$sqlstudyPop="SELECT * FROM ".$prefix."study_population where `owner_id`='$asrmApplctID' and protocol_id='$id' order by id desc limit 0,1";//and status='new' 
...
$sqlEwCC = "select * FROM ".$prefix."submission_upload where user_id='$asrmApplctID' and submission_id='$id' and upload_type_id='3'";//and conceptm_status='new' 
$resultWeCC = $mysqli->query($sqlEwCC);
$totalUserErCC = $resultWeCC->num_rows;//for only clinical trials 2. Informed Consent Forms (for Clinical Trials)<br>

///////////////////////////////////////////////////////////////
$sqlEwCC22 = "select * FROM ".$prefix."submission_upload where user_id='$asrmApplctID' and submission_id='$id' and upload_type_id='28'";//and conceptm_status='new' 
$resultWeCC22 = $mysqli->query($sqlEwCC22);
$totalUserErCC22 = $resultWeCC22->num_rows;

$sqlEwCC24 = "select * FROM ".$prefix."submission_upload where user_id='$asrmApplctID' and submission_id='$id' and upload_type_id='3'";//informed concent
$resultWeCC24 = $mysqli->query($sqlEwCC24);
$totalUserErCC44 = $resultWeCC24->num_rows;
...
//
$wm="select * from ".$prefix."submission_stages where  owner_id='$asrmApplctID' and protocol_id='$id' and status='new'";
$cmdwb = $mysqli->query($wm);
$totalStages = $cmdwb->num_rows;
$r= $cmdwb->fetch_array();


if($totalStages and $totalUserEr and $totalUserErCC22 || $required>=1){
$sqlASubmissionStages="update ".$prefix."submission_stages  set `filem`='1' where `owner_id`='$asrmApplctID' and `protocol_id`='$id'";
$mysqli->query($sqlASubmissionStages);
}else{
$sqlASubmissionStages="update ".$prefix."submission_stages  set `filem`='0' where `owner_id`='$asrmApplctID' and `protocol_id`='$id'";
$mysqli->query($sqlASubmissionStages);
}

if($totalUserEr and $totalUserErCC22 and $is_clinical_trial==1 and $totalUserErCC44){ //and $totalUserErCC Ignore 2. Informed Consent Forms (for Clinical Trials)<br>
?>   
