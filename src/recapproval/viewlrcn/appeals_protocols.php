...
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submission where is_sent='1' and recAffiliated_id='$recAffiliated_id' and (public_title like '%$searchprotocol%' OR code like '%$searchprotocol%' OR shtname like '%$searchprotocol%') and status='$status' and  	appeals='Yes' order by id desc");//and conceptm_status='new' 
}

if($_POST['status'] and !$_POST['searchprotocol']){	
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submission where is_sent='1' and recAffiliated_id='$recAffiliated_id' and  status='$status' and recruitment_status_id='1' order by id desc");//and conceptm_status='new' 
}

if(!$_POST['status'] and $_POST['searchprotocol']){	
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submission where is_sent='1' and recAffiliated_id='$recAffiliated_id' and  (public_title like '%$searchprotocol%' OR code like '%$searchprotocol%' OR shtname like '%$searchprotocol%') and appeals='Yes' order by id desc");//and conceptm_status='new' 
...
if(!$_POST['doSearch']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submission where is_sent='1' and recAffiliated_id='$recAffiliated_id' and appeals='Yes' order by id desc");//and conceptm_status='new' 
}
...
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."submission where is_sent='1'  and recAffiliated_id='$recAffiliated_id' and (public_title like '%$searchprotocol%' OR code like '%$searchprotocol%' OR shtname like '%$searchprotocol%') and status='$status' and appeals='Yes' order by id desc LIMIT $start, $limitm";//and conceptm_status='new' 
}//'Pending','reviewed','Approved','Rejected'

if($_POST['status'] and !$_POST['searchprotocol']){							//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."submission where is_sent='1'  and recAffiliated_id='$recAffiliated_id' and status='$status' and appeals='Yes' order by id desc LIMIT $start, $limitm";//and conceptm_status='new' 
}//'Pending','reviewed','Approved','Rejected'

if(!$_POST['status'] and $_POST['searchprotocol']){							//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."submission where is_sent='1'  and recAffiliated_id='$recAffiliated_id' and (public_title like '%$searchprotocol%' OR code like '%$searchprotocol%' OR shtname like '%$searchprotocol%') and appeals='Yes' order by id desc LIMIT $start, $limitm";//and conceptm_status='new' 
}//'Pending','reviewed','Approved','Rejected'

if(!$_POST['doSearch']){
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."submission where is_sent='1'  and recAffiliated_id='$recAffiliated_id' and appeals='Yes' order by id desc LIMIT $start, $limitm";//and conceptm_status='new'
}
...
<input id="go" type="button" value="APPEAL ACCEPTED" onClick="window.location.href='./main.php?option=viewsubmissionrec&id=<?php echo $rProjects['id'];?>'" class="errormm4">
...
<?php }?>
                          <?php /*?><?php if($rProjects['assignedto']){?> 
                           ...
                           <?php if($rProjects['recruitment_status_id']=='1'){?>
