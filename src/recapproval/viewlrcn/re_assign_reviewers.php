...
<li><a href="#" rel="#default" class="selected">Re-Assign Reviewers</a></li>
...
$asrmApplctID_user=$mysqli->real_escape_string($_POST['asrmApplctID_user']);
	...
	if(!$_POST['Meetingsubject']){
	$subject=$mysqli->real_escape_string("Re-Assigning of protocol");
...
$sqlA2rr="insert into ".$prefix."submission_review_sr (`asrmApplctID`,`protocol_id`,`owner_id`,`reviewer_id`,`reviewDate`,`recstatus`,`protocolStage`,`reviewtype`,`subject`,`recAffiliated_c`,`reviewFor`,`conflictofInterest`,`conflictReason`,`reviewStatus`,`reassigned`) 

values('$cfnreviewer','$protocol_idmm','$asrmApplctID_user','$cfnreviewer',now(),'new','stage1','$reviewtype','$subject','$recAffiliated_c','protocol','none','','Pending','Yes')";
...
//require_once("assign_protocol_to_tertiary_reviews.php");
}
...
$sqlProtocols="SELECT * FROM ".$prefix."submission_review_sr  where protocol_id='$id' and recstatus='new' and reassigned='Yes' order by id desc";
...
while($sqProtocols = $QueryProtocols->fetch_array()){
	...
	$update="update ".$prefix."submission_review_sr set recstatus='NotReviewed',reassigned='Yes' where protocol_id='$id' and recstatus='new' and reassigned!='Yes'";
	$mysqli->query($update);
	//Get Reviewer
...
<?php /*?><?php if($rstudym['type_of_review']!='Expedited Review' and $rstudym['type_of_review']!='Fast Track' and $rstudym['type_of_review']!='Exempt'){?>
  <div class="form-group row">
...
//if($rstudym['type_of_review']=='Expedited Review' || $rstudym['type_of_review']=='Fast Track' || $rstudym['type_of_review']=='Exempt'){
?>
