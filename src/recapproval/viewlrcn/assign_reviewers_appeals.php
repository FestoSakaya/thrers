...
$asrmApplctID_user=$mysqli->real_escape_string($_POST['asrmApplctID_user']);
	...
	$subject="Review of protocol Appeal";
...
values('$cfnreviewer','$protocol_idmm','$asrmApplctID_user','$cfnreviewer',now(),'new','stage1','$reviewtype','$subject','$recAffiliated_c','HaltedAppeal','none')";
...
$update="update ".$prefix."appeal_halted_studies set status='Scheduled for Review',assignedto='Assigned' where id='$act' and protocol_id='$protocol_idmm' and owner_id='$asrmApplctID_user'";
$mysqli->query($update);
///Now Send mail
require_once("viewlrcn/mail_template_assign_reviewers_appeal.php");
...
$mail->Subject = "Protocol Appeal for Review";
$body="$allSentMessage";
$mail->MsgHTML($body);
...
echo '<meta http-equiv="refresh" content="2; url='.$base_url.'/main.php?option=HaltedStudies" />';
}
?>
<!-- Project-->
              <div class="project">
...
<h3 class="success">Protocol Appeal for <?php echo $rstudym['public_title'];?></h3>
...
<?php
$sqlProtocols="SELECT * FROM ".$prefix."submission_review_sr  where protocol_id='$id' and recstatus='new'";
