...
<button class="accordion">Study Description, click to review</button>
  ...
  <table id="customers">
                        ...
                        <?php
//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."submission_country where submission_id='$protocol_idwe' and owner_id='$owner_id' order by id desc LIMIT 0,200";//and conceptm_status='new' $owner_id
$result = $mysqli->query($sql);
while($rInvestigator=$result->fetch_array()){
...
<label class="form-control-label"><strong style="font-weight:bold;">Ethical Considerations:</strong> <?php echo nl2br($rstudymethodology['ethical_considerations']);?></label>
 ...
 <button class="accordion">Study Work Plan, click to review</button>
  ...
  <table id="customers">
                        ...
                        <?php
//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%Y-%m-%d') AS created,DATE_FORMAT(`updated`,'%d/%m/%Y') AS updated FROM ".$prefix."submission_task where submission_id='$protocol_idwe' and owner_id='$owner_id' order by id desc LIMIT 0,10";//and conceptm_status='new'  %H:%s:%i
$result = $mysqli->query($sql);
while($rInvestigator=$result->fetch_array()){
	?>
                          <tr>
...
<?php if($today<=$rInvestigator['created']){?>
<a href="./cfxdownload.php?bkey=<?php echo $rInvestigator['id'];?>" target="_blank"><?php echo $rInvestigator['description'];?></a>
<?php }else{?>
<a href="./cfxdownload.php?bkey=<?php echo $rInvestigator['id'];?>" target="_blank"><?php echo $rInvestigator['description'];?></a>
<?php }?> 
  ...
  <table id="customers">
                        ...
                        <?php
//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%Y-%m-%d') AS created FROM ".$prefix."submission_upload where user_id='$owner_id' and submission_id='$protocol_idwe' order by id desc LIMIT 0,150";//and conceptm_status='new' 
$result = $mysqli->query($sql);
while($rInvestigator=$result->fetch_array()){
...
$rUsers=$resultUser->fetch_array();
	?>
                          <tr>
                            ...
                            <?php if($today<=$rInvestigator['created'] and $rInvestigator['filename']){?>
...
<?php }?>
   ...
   $sqlstudyff="SELECT *,DATE_FORMAT(`created`,'%Y-%m-%d') AS created FROM ".$prefix."submission where id='$protocol_idwe' order by id desc limit 0,1";
$Querystudyff = $mysqli->query($sqlstudyff);
$rstudyff = $Querystudyff->fetch_array(); 
...
$rUsers2=$resultUser2->fetch_array();
 ?>  
     ...
     <tr>
                            <td><?php if($today<=$rstudyff['created']){?>
<a href="./cfxdownload.php?n=<?php echo $rstudyff['id'];?>" target="_blank">Proof of Payment</a>
<?php }else{?>
<a href="./cfxdownload.php?n=<?php echo $rstudyff['id'];?>" target="_blank">Proof of Payment</a>
<?php }?> </td>
                            <td>Payment</td>
...
<!--  <button class="accordion">History, click to review</button>
 ...
 <?php if($_SESSION['privillage']=='recreviewer' || $session_privillage=='recadmin' || $session_privillage=='rechairperson' || $session_privillage=='revicechairperson' || $session_privillage=='recitadmin'){?>
...
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."initial_committee_screening where reviewer_id='$asrmApplctID' and protocol_id='$protocol_idwe' order by id desc LIMIT 0,100";//and conceptm_status='new'
}
...
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."initial_committee_screening where owner_id='$owner_id' and protocol_id='$protocol_idwe' order by id desc LIMIT 0,100";//and conceptm_status='new'
}
