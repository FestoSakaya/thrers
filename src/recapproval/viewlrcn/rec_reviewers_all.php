...
$adjacents = 3;
if($_POST['searchname']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."user where name like '%$name%' and recAffiliated_id='$id' and (privillage='recreviewer') order by asrmApplctID desc");//and conceptm_status='new'
}
if(!$_POST['searchname']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."user where recAffiliated_id='$id' and (privillage='recreviewer') order by asrmApplctID desc");//and conceptm_status='new'
}
...
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."user where name like '%$name%' and recAffiliated_id='$id' and (privillage='recreviewer')  order by asrmApplctID desc LIMIT 0,100";//and conceptm_status='new'
}
if(!$_POST['searchname']){//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."user where recAffiliated_id='$id' and (privillage='recreviewer') order by asrmApplctID desc LIMIT 0,100";//and conceptm_status='new'$start, $limitm
}
...
$sqUserRec = $resultSSS->fetch_array();
	?>
                          <tr>
                            ...
                            <td><?php echo $rProjects['privillage'];?>
