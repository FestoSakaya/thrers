...
<input name="" type="button" class="search dropbtn2" value="Click to submit Renewals" onClick="window.location.href='./main.php?option=SAnnualRenewals&id=<?php echo $id;?>'"/>
...
<div class="card-header d-flex align-items-center">
               ...
               <table width="100%" class="table table-striped table-sm" id="customers">
                        <thead>
                          <tr>
                            <th width="367">Project Title </th>
                            <th width="100">Enrolled No</th>
                            <th width="153">Sample size</th>
...
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."renewals where (Briefrationale like '%$searchprotocol%' OR PDDescriptionofdeviation like '%$searchprotocol%' OR Approvedsamplesize_Number like '%$searchprotocol%') and owner_id='$asrmApplctID' order by id desc");//and conceptm_status='new' 
}
if(!$_POST['searchprotocol']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."renewals where owner_id='$asrmApplctID' order by id desc");//and conceptm_status='new' 
}
...
$sqprotocol = $resultprotocol->fetch_array();
	...
	?>
                          <tr>
                            <td><h3 class="h4">
							<?php if($rProjects['ammendType']=='manual'){?><?php echo $rProjects['public_title'];}?>
                            
                            <?php if($rProjects['ammendType']=='online'){?><?php echo $sqprotocol['public_title'];}?>
							</h3></td>
                            ...
                            <td><a href="./main.php?option=viewRenewals&id=<?php echo $rProjects['id'];?>"><span class="label label-primary">+ View Submission</span></a><div style="margin-bottom:6px;"></div>
                            
                 <?php if($rProjects['is_sent']==0 and $rProjects['ammendType']=='manual'){?>  <a href="./main.php?option=AnnualRenualManual&id=<?php echo $rProjects['id'];?>"><span class="label label-secwarning">+ Edit Submission</span></a>  <?php }?>       
              
                   <?php if($rProjects['is_sent']==0 and $rProjects['ammendType']=='online'){?>  <a href="./main.php?option=AnnualRenual&id=<?php echo $rProjects['id'];?>"><span class="label label-secwarning">+ Edit Submission</span></a>  <?php }?>             
