...
<input name="" type="button" class="search dropbtn2" value="Click to submit Notifications" onClick="window.location.href='./main.php?option=NotificationsBegin'"/>
...
<div class="card-header d-flex align-items-center">
                      <h3 class="h4">Safety and Other Notifications </h3>
</div>
                    <div class="card-body">
               <table width="100%" class="table table-striped table-sm" id="customers">
                        <thead>
                          <tr>
                            <th width="461">Project Title </th>
                            <th width="156">Attachment</th>
                            <th width="183">Submission Date</th>
                                      
                            <th width="89">Status</th>
                            <th width="182">Actions</th>
                          </tr>
                        </thead>
                        <tbody>
                        <?php
...
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."notifications  where (fileAttachment like '%$searchprotocol%') and owner_id='$asrmApplctID' order by id desc");//and conceptm_status='new' 
}
if(!$_POST['searchprotocol']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."notifications  where owner_id='$asrmApplctID' order by id desc");//and conceptm_status='new' 
}
...
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS datesubmitted FROM ".$prefix."notifications  where (fileAttachment like '%$searchprotocol%') and owner_id='$asrmApplctID' order by id desc LIMIT $start, $limitm";//and conceptm_status='new'
}
if(!$_POST['searchprotocol']){							//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS datesubmitted FROM ".$prefix."notifications  where owner_id='$asrmApplctID' order by id desc LIMIT $start, $limitm";//and conceptm_status='new'
}
...
$sqlstudy="SELECT *,DATE_FORMAT(`created`,'%Y-%m-%d') AS created FROM ".$prefix."notifications_attachments where notification_id='$notification_id' order by id desc";
...
<?php }?>
                            ...
                            <td>                            
                  <?php if($rProjects['status']=='Pending'){?> <a href="./main.php?option=Notifications&id=<?php echo $protocol_id;?>"><span class="label label-primary">+ Edit Submission</span></a> <?php }?>
