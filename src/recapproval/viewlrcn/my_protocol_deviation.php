...
<input name="" type="button" class="search dropbtn2" value="Click to submit Deviations" onClick="window.location.href='./main.php?option=DeviationsStepOne'"/>
...
<div class="card-header d-flex align-items-center">
               ...
               <table width="100%" class="table table-striped table-sm" id="customers">
                        <thead>
                          <tr>
                            <th width="767">Project Title </th>
                            <th width="197">Submission Date</th>
                          
                            <th width="176">Status</th>
                            <th width="189">Actions</th>
                          </tr>
                        </thead>
                        <tbody>
                        <?php
...
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."deviations where (PDDateofoccurrence like '%$searchprotocol%' OR PDDescriptionofdeviation like '%$searchprotocol%' OR Rootcauseofdeviation like '%$searchprotocol%') and owner_id='$asrmApplctID' order by deviationID desc");//and conceptm_status='new' 
}
if(!$_POST['searchprotocol']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."deviations where owner_id='$asrmApplctID' order by deviationID desc");//and conceptm_status='new' 
}
...
$sql = "select *,DATE_FORMAT(`updatedon`,'%d/%m/%Y %H:%s:%i') AS datesubmitted FROM ".$prefix."deviations where (PDDateofoccurrence like '%$searchprotocol%' OR PDDescriptionofdeviation like '%$searchprotocol%' OR Rootcauseofdeviation like '%$searchprotocol%') and owner_id='$asrmApplctID' order by deviationID desc LIMIT $start, $limitm";//and conceptm_status='new'
}
if(!$_POST['searchprotocol']){							//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`updatedon`,'%d/%m/%Y %H:%s:%i') AS datesubmitted FROM ".$prefix."deviations where owner_id='$asrmApplctID' order by deviationID desc LIMIT $start, $limitm";//and conceptm_status='new'
}
...
$sqprotocol = $resultprotocol->fetch_array();
	...
	?>
                          <tr>
                            ...
                            <td><a href="./main.php?option=viewDeviation&id=<?php echo $rProjects['deviationID'];?>"><span class="label label-primary">+ View Submission</span></a><div style="margin-bottom:6px;"></div>
                            
<?php if($rProjects['is_sent']=='0' and $rProjects['ammendType']=='manual'){?><a href="./main.php?option=DeviationsManual&id=<?php echo $rProjects['deviationID'];?>"><span class="label label-secwarning">+ Edit Submission</span></a>  <?php }?> 

<?php if($rProjects['is_sent']=='0' and $rProjects['ammendType']=='online'){?><a href="./main.php?option=DeviationsOnline&id=<?php echo $rProjects['deviationID'];?>"><span class="label label-secwarning">+ Edit Submission</span></a>  <?php }?>       
                       <?php /*?>DeviationsManual
                            <?php if($rProjects['status']=='pending'){?><a href="./main.php?option=submission/<?php echo $rProjects['id'];?>/" style="color:#06F; font-weight:bold;">+ Edit Submission</a>
			<div style="margin-bottom:6px;"></div>				
		<a href="./main.php?option=dashboardDel/<?php echo $rProjects['id'];?>/" style="color:#F00; font-weight:bold;" onclick="return confirm('Are you sure you want to delete this item?');">+ Delete Submission</a>					
