...
<div class="card-header d-flex align-items-center">
                      <h3 class="h4">Ammendments for Review</h3>
</div>
                    <div class="card-body">
...
$adjacents = 3;
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submission_review_sr where reviewer_id='$asrmApplctID' and reviewFor='Amendments' order by id desc");//and conceptm_status='new'
...
$sql = "select * FROM ".$prefix."submission_review_sr where  reviewer_id='$asrmApplctID'  and reviewFor='Amendments' order by id desc LIMIT $start, $limitm";//and conceptm_status='new'
...
$sqlstudym="SELECT *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."ammendments where id='$renewal_id' order by id desc limit 0,1";
...
$rstudym = $Querystudym->fetch_array();
	...
	?>
                          <tr>
                            <td scope="row">
							<?php if($rstudym['ammendType']=='manual'){echo $rstudym['refNo'];}?>
 <?php if($rstudym['ammendType']=='online'){echo $sqReview['code'];}?>
                            ...
                            <small><?php echo $sqUserdd['name'];?> - <?php echo $sqUserdd['institution'];?></small></td>
                            <td><?php if($rProjects['ammendType']=='manual'){?><span class="label label-warning">Manual</span> <?php }?>
  <?php if($rProjects['ammendType']=='online'){?><span class="label label-primary">Online</span> <?php }?>                          </td>
   <td><?php echo $sqUserddRREC['name'];?></td>
                            <td><?php echo $rstudym['created'];?></td>
                            <td>
                            ...
                            <td>
<?php if($rProjects['recstatus']=='Pending' || $rProjects['recstatus']=='new' and $rProjects['conflictofInterest']=='no'){?><a href="./main.php?option=ReviewerAmmendmnets&id=<?php echo $rProjects['id'];?>" style=" color:#ffffff;" class="label label-sec4">Review Submission</a><br />

<?php }?>

<a href="./main.php?option=viewRevAmmendments&id=<?php echo $rProjects['id'];?>" style="color:#06F; font-weight:bold;">View Details</a>
...
<?php /*?><?php if($rProjects['recstatus']!='Pending' and $rProjects['conflictofInterest']=='no'){?><a href="<?php echo $base_url;?>recapprova./main.php?option=dashboard" onClick="return popitup('comments.php?id=<?php echo $main_submission_id;?>')" class="label label-sec2" style="padding-left:15px; padding-right:13px;">View Comments</a><div style="margin-bottom:6px;"></div><?php }?><?php */?>
