...
<table width="100%" border="0" style="margin-top:15px;">
  <tr>
    <td width="59%">Find Notifications:<br />
...
<div class="card-header d-flex align-items-center">
                      <h3 class="h4">Safety and Other Safety and Other Notifications   </h3>
</div>
                    <div class="card-body">
               <table width="100%" class="table table-striped table-sm" id="customers">
                        <thead>
                          <tr>
                            <th width="480">Project Title </th>
                            <th width="156">Attachment</th>
                            <th width="152">Submission Date</th>
                                      
                            <th width="78">Status</th>
                            <th width="188">Actions</th>
                          </tr>
                        </thead>
                        <tbody>
                        <?php
...
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."notifications  where (fileAttachment like '%$searchprotocol%') and recAffiliated_id='$recAffiliated_id' order by id desc");//and conceptm_status='new' 
}
if(!$_POST['searchprotocol']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."notifications  where recAffiliated_id='$recAffiliated_id' order by id desc");//and conceptm_status='new' 
}
...
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS datesubmitted FROM ".$prefix."notifications  where (fileAttachment like '%$searchprotocol%') and recAffiliated_id='$recAffiliated_id' order by id desc LIMIT $start, $limitm";//and conceptm_status='new'
}
if(!$_POST['searchprotocol']){							//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS datesubmitted FROM ".$prefix."notifications  where recAffiliated_id='$recAffiliated_id' order by id desc LIMIT $start, $limitm";//and conceptm_status='new'
}
...
$sqUserMeeting = $resultSMeeting->fetch_array();
	...
	?>
                          <tr>
							...
							<?php echo $sqprotocol['public_title'];}?>
                            
                            <?php if($rProjects['ammendType']=='manual'){?>
							<?php echo $rProjects['public_title'];}?>
                            </h3></td>
                            <td>
...
$sqlstudy="SELECT * FROM ".$prefix."notifications_attachments where notification_id='$notification_id' order by id desc";
...
<?php }?>
                            ...
                            <td>                            
         <a href="./main.php?option=ConfirmNotifications&id=<?php echo $rProjects['id'];?>"><span class="label label-primary">+ View Submission</span></a><div style="margin-bottom:6px;"></div>
                            
                           
                <div style="margin-bottom:6px;"></div>                         
<?php 
if($rProjects['assignedto']=='Not Assigned' and $rProjects['status']=='Submitted' and $session_privillage=='recadmin'){?><a href="./main.php?option=ConfirmNotifications&id=<?php echo $rProjects['id'];?>"><span class="label label-sec2">Assign Reviewers</span></a><div style="margin-bottom:6px;"></div><?php } //$rProjects['paymentProof'] and?>    
                    
<?php 
	if($rProjects['status']!='Approved' and $rProjects['assignedto']=='Assigned' and $session_privillage=='recadmin'){?>
  <a href="./main.php?option=ConfirmNotificationsFinal&id=<?php echo $rProjects['id'];?>"><span class="label label-sec2">Make Final Decision</span></a><div style="margin-bottom:6px;"></div><?php }?> 
