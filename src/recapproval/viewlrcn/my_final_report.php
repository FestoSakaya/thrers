...
<input name="" type="button" class="search dropbtn2" value="Click to submit Closeout Reports" onClick="window.location.href='./main.php?option=ApplyFinalReport'"/>
...
<div class="card-header d-flex align-items-center">
               ...
               <table width="100%" class="table table-striped table-sm" id="customers">
                        <thead>
                          <tr>
                            <th width="449">Project Title </th>
                            <th width="203">Attachment</th>
                            <th width="154">Submission Date</th>
                                      
                            <th width="88">Status</th>
                            <th width="177">Actions</th>
                          </tr>
                        </thead>
                        <tbody>
                        <?php
...
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."final_reports  where (fileAttachment like '%$searchprotocol%') and owner_id='$asrmApplctID' order by id desc");//and conceptm_status='new' 
}
if(!$_POST['searchprotocol']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."final_reports  where owner_id='$asrmApplctID' order by id desc");//and conceptm_status='new' 
}
...
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS datesubmitted FROM ".$prefix."final_reports  where (fileAttachment like '%$searchprotocol%') and owner_id='$asrmApplctID' order by id desc LIMIT $start, $limitm";//and conceptm_status='new'
}
if(!$_POST['searchprotocol']){							//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS datesubmitted FROM ".$prefix."final_reports  where owner_id='$asrmApplctID' order by id desc LIMIT $start, $limitm";//and conceptm_status='new'
}
...
$sqlstudy="SELECT *,DATE_FORMAT(`created`,'%Y-%m-%d') AS created FROM ".$prefix."final_reports_attachments where notification_id='$notification_id' order by id desc";
...
<?php }?>
                            ...
                            <td>                            
                  <?php if($rProjects['is_sent']=='0' and $rProjects['ammendType']=='online'){?> <a href="./main.php?option=OnlineFinalReport&id=<?php echo $rProjects['id'];?>"><span class="label label-primary">+ Edit Submission</span></a> <?php }?>
                            

 <?php if($rProjects['is_sent']=='0' and $rProjects['ammendType']=='manual'){?> <a href="./main.php?option=ManualFinalReport&id=<?php echo $rProjects['id'];?>"><span class="label label-primary">+ Edit Submission</span></a> <?php }?>
