...
<html>
  ...
  <base href="<?php echo $base_url;?>">
   ...
   <!-- <script src="https://use.fontawesome.com/99347ac47f.js"></script> commented out-->
    <!-- Font Icons CSS-->
 <!--   <link rel="stylesheet" href="https://file.myfontastic.com/da58YPMQ7U5HY8Rb6UxkNf/icons.css"> commented out-->
...
?>
       ...
       <?php
if($_POST['doTeam']=='Save' and $_POST['investigator']){
...
$targetw1 = "/var/www/clients/client1/web1/tmp/uploads/". basename($sasrmApplctID.preg_replace('/\s+/', '_', $_FILES['GCPtraining']['name']));
$studytoolsext_main1 = move_uploaded_file(preg_replace('/\s+/', '_', $_FILES['GCPtraining']['tmp_name']), $targetw1);

$sqlA2ww="update ".$prefix."team set `GCPtraining`='$GCPtraining2' where id='$id'";
...
$sqlA2="update ".$prefix."team set `name`='$investigator',`institution`='$institution',`email`='$email',`countryid`='$countryid',`project_role`='$project_role',`Telephone`='$Telephone',`qualification`='$qualification' where id='$id'";
$mysqli->query($sqlA2);

$sqlASubmissionStages="update ".$prefix."submission_stages  set `protocol_information`='0' where `owner_id`='$sasrmApplctID' and `protocol_id`='$record_id'";
...
if($_POST['rstug_educn_designation']){
...
$sqlEduc="SELECT * FROM ".$prefix."education_history where `rstug_user_id`='$sasrmApplctID' and teamMemberID='$id' and rstug_educn_university='$rstug_educn_university' and  rstug_educn_designation='$rstug_educn_designation' and  rstug_educn_year='$rstug_educn_year' order by rstug_educn_id desc limit 0,1";
$QueryEduc = $mysqli->query($sqlEduc);
$totalEduc = $QueryEduc->num_rows;

if(strlen($_POST['rstug_educn_designation'][$i])>=5 and !$totalEduc){
$Insert_QR2="insert into ".$prefix."education_history (`rstug_user_id`,`rstug_educn_university`,`rstug_educn_designation`,`rstug_educn_year`,`completionyear`,`rstug_educn_period`,`rstug_ammend`,`rstug_Synched`,`teamMemberID`) values ('$sasrmApplctID','$rstug_educn_university','$rstug_educn_designation','$rstug_educn_year','$completionyeareduc','$rstug_educn_period','0','0','$id')";
$mysqli->query($Insert_QR2);
$record_id1 = $mysqli->insert_id;
}

if($record_id1){
$sqlA2Protocolamm="update ".$prefix."team  set `education`='1' where owner_id='$sasrmApplctID' and id='$id'";
...
if($_POST['rstug_institution']){ 
for ($i=0; $i < count($_POST['rstug_institution']); $i++) {
...
$sqlEmploy="SELECT * FROM ".$prefix."employment_details where `rstug_user_id`='$sasrmApplctID' and teamMemberID='$id' and rstug_institution='$institution' and rstug_designation='$designation' and rstug_year='$year' order by rstug_employment_id desc limit 0,1";
$QueryEmploy = $mysqli->query($sqlEmploy);
$totalEmploy = $QueryEmploy->num_rows;


if(strlen($_POST['rstug_institution'][$i])>=5 and !$totalEmploy){
$Insert_QR2="insert into ".$prefix."employment_details (`rstug_user_id`,`rstug_institution`,`rstug_designation`,`rstug_year`,`completionyear`,`rstug_period`,`rstug_ammend`,`teamMemberID`) values ('$sasrmApplctID','$institution','$designation','$year','$completionyear','$period','0','$id')";
$inseted=$mysqli->query($Insert_QR2);
$record_id2 = $mysqli->insert_id;
}
//////////////update
if($record_id2){
$sqlA2Protocola="update ".$prefix."team  set `employment`='1' where owner_id='$sasrmApplctID' and id='$id'";
...
$titledd=$mysqli->real_escape_string(htmlentities($_POST['title'][$i]));
$citationdd=$mysqli->real_escape_string(htmlentities($_POST['citation'][$i]));
...
$Insert_QR2ff="insert into ".$prefix."publications (`owner_id`,`protocol_id`,`title`,`citation`,`created`,`teamMemberID`) values ('$sasrmApplctID','','$titledd','$citationdd',now(),'$id')";
...
<?php
$sqlstudym="SELECT * FROM ".$prefix."team where `id`='$id' order by id desc limit 0,1";
...
<form action="" method="post" name="regForm" id="regForm" autocomplete="off" enctype="multipart/form-data">
 ...
 <table width="100%" border="0" class="success"  style="margin-left:10px;">
  <tr>
    <td width="50%"> <div class="form-group row" style="padding-top:6px;">

<div class="col-sm-10">
<label class="col-sm form-control-label"><strong>Name: <span class="error">*</span></strong></label>
<input type="text" name="investigator" id="investigator" class="form-control  required" value="<?php echo $rstudym['name'];?>" placeholder="First Name" required autocomplete="off">
...
<input type="text" name="qualification" id="investigator" class="form-control  required" placeholder="Qualification" value="<?php echo $rstudym['qualification'];?>" required autocomplete="off">
</div>
</div></td>
  </tr>
  <tr>
...
<option value="Team Member" <?php if($rstudym['project_role']=='Team Member'){?>selected<?php }?>>Team Member</option>
<option value="Principal Investigator" <?php if($rstudym['project_role']=='Principal Investigator'){?>selected<?php }?>>Principal Investigator</option>
<option value="Co-Investigator" <?php if($rstudym['project_role']=='Co-Investigator'){?>selected<?php }?>>Co-Investigator</option>
...
<input type="text" name="institution" id="institution" class="form-control  required" value="<?php echo $rstudym['institution'];?>" placeholder="Institution" required minlength="10" autocomplete="off">
...
<input type="email" name="email" id="email" class="form-control  required email" value="<?php echo $rstudym['email'];?>" placeholder="Email" required>
</div>
</div></td>
    <td width="50%"><div class="form-group row">
...
<input type="text" name="Telephone" id="email" class="form-control  required" value="<?php echo $rstudym['Telephone'];?>" placeholder="Telephone" required autocomplete="off">
</div>
</div> </td>
  </tr>
  <tr>
...
<?php if($rstudym['GCPtraining']){?>
<input name="GCPtraining" type="file" id="GCPtraining" />
<a href="./files/uploads/<?php echo $rstudym['GCPtraining'];?>" style="color:#06F; font-weight:bold; font-size:12px;" target="_blank">View File</a><?php }?>
...
?>
<option value="<?php echo $rCountrycv['id'];?>" <?php if($rCountrycv['id']==$rstudym['countryid']){?>selected="selected"<?php }?>><?php echo $rCountrycv['name'];?></option>
<?php }?>
</select>
</div>
</div> </td>
...
<h3 style="font-size:14px; font-weight:bold;" class="defmf">Employment <span class="error">*</span></h3><hr />
<table width="80%" border="0" id="POITable" class="thhdeaders success">
        <tr>
        ...
        <tr>
            ...
            <td>
                   <input type="text" name="rstug_institution[]" id="cvd" tabindex="8" class="requiremd" minlength="5" style="border:1px solid #7F9DB9;width:160px;background:#ffffff;padding:5px;" value=""/>
            

            </td>
            <td><input type="text" name="designation[]" id="customss2" tabindex="5" class="requiredm" style="border:1px solid #7F9DB9;width:200px;background:#ffffff;padding:5px;" value=""/></td>
...
<?php }?>
            ...
            <select name="completionyear[]" id="ssss" class="requireds" style="border:1px solid #7F9DB9;background:url(./images/fmbg.jpg);padding:5px; width:100px;" >
<option value="">Year</option>
...
<tr>
            ...
            <td><a href="<?php echo $base_url;?>recapproval/team.php?id=<?php echo $id;?>&emdcon=del&em=<?php echo $rowRows2['rstug_employment_id'];?>" style="color:#F00; font-weight:bold;" onclick="return confirm('Are you sure you want to delete this item?');">Delete</a></td>
        </tr><?php } ?> 
<?php } ?> 
   ...
   <h3 style="font-size:14px; font-weight:bold;" class="defmf">Education <span class="error">*</span></h3> <hr />
    <table width="100%" >
                <tr>
<td colspan="2">

<table width="80%" border="0" id="POITable2" class="thhdeaders success">
        <tr>
...
<?php }?>
       ...
       <select name="completionyeareduc[]" id="ssss" class="requireds" style="border:1px solid #7F9DB9;background:url(./images/fmbg.jpg);padding:5px; width:100px;" >
<option value="">Year</option>
<?php
define('DOB_YEAR_START', $startYear);
...
<tr>
            ...
            <td><?php echo $rowRows['rstug_educn_period'];?> 
                 ...
                 <td><a href="<?php echo $base_url;?>recapproval/team.php?id=<?php echo $id;?>&emdeduc=del&em=<?php echo $rowRows['rstug_educn_id'];?>" style="color:#F00; font-weight:bold;" onclick="return confirm('Are you sure you want to delete this item?');">Delete</a></td>
                 <td>&nbsp;</td>
        </tr>
...
<?php }//end totas?> 
   ...
   <h3 style="font-size:14px; font-weight:bold; margin-top:10px;" class="defmf">Recent Publications (Last 5 Years)</h3> <hr />
    <table width="100%"  class="success">
                <tr>
<td colspan="2">
...
<tr>
            ...
            <td><?php echo $rowRows['citation'];?> </td>
                 ...
                 <td colspan="2"><a href="<?php echo $base_url;?>recapproval/team.php?id=<?php echo $id;?>&emdcitation=del&em=<?php echo $rowRows['id'];?>" style="color:#F00; font-weight:bold;" onclick="return confirm('Are you sure you want to delete this item?');">Delete</a></td>
        </tr>
