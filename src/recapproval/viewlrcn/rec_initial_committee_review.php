...
<li><a href="#" rel="#default" class="selected">Reviewer Comments</a></li>
<li class="extra">Opinion</li>
</ul>
...
if($_POST['doSendToEthical']=='Save' and $_POST['screening']){
...
$studytoolsext_main1 = move_uploaded_file(preg_replace('/\s+/', '_', $_FILES['draftopinion']['tmp_name']), $targetw1);
	...
	$sqlInvestigators="SELECT * FROM ".$prefix."initial_committee_screening where `owner_id`='$asrmApplctID_user' and screening='$screening' and protocol_id='$protocol_idmm' and reviewer_id='$session_reviewer' order by id desc";
	$QueryInvestigators = $mysqli->query($sqlInvestigators);
	$totalInvestigators = $QueryInvestigators->num_rows;
	
		if(!$totalInvestigators){
$sqlA2="insert into ".$prefix."initial_committee_screening (`owner_id`,`protocol_id`,`created`,`updated`,`screening`,`draftopinion`,`recAffiliated_id`,`reviewer_id`,`screeningFor`) 

values('$asrmApplctID_user','$protocol_idmm',now(),now(),'$screening','$draftopinion2','$recAffiliated_c','$session_reviewer','protocol')";
$mysqli->query($sqlA2);

//$sqlA2rr="insert into ".$prefix."submission_review_sr (`asrmApplctID`,`protocol_id`,`owner_id`,`reviewer_id`,`reviewDate`,`recstatus`,`protocolStage`,`reviewtype`,`subject`,`recAffiliated_c`) values('$asrmApplctID','$protocol_idmm','$asrmApplctID_user','$session_reviewer',now(),'Pending','stage1','','','$recAffiliated_c')";
//$mysqli->query($sqlA2rr); Dont insert, it creates duplicates
		}
echo '<img src="images/loading_wait.gif">';
echo '<div class="spacer"></div>';

echo '<meta http-equiv="REFRESH" content="0;url='.$base_url.'/main.php?option=recEndReview&id='.$id.'&srid='.$srid.'">';
...
<hr /> <table class="table table-striped table-sm">
                        <thead>
                          <tr>
                            <th>Registry Name</th>
...
$mysqli->query($sqlA2);
    ...
    <table class="table table-striped table-sm">
                        ...
                        <?php
//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."protocol_comment where user_id='$owner_id' and protocol_id='$protocol_idwe' order by id desc LIMIT 0,10";//and conceptm_status='new' 
$result = $mysqli->query($sql);
while($rInvestigator=$result->fetch_array()){
$upload_type_id=$rInvestigator['upload_type_id'];
$submittedBy=$rInvestigator['user_id'];
...
<input name="asrmApplctID_user" type="text" value="<?php echo $owner_id;?>"/>
<input name="protocol_idmm" type="text" value="<?php echo $protocol_idwe;?>"/>
