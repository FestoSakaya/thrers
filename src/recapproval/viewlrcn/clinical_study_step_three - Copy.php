...
$sqlA2="insert into ".$prefix."submission_country (`owner_id`,`submission_id`,`country_id`,`district_id`,`created`,`updated`,`participants`,`Municipality`,`SubCounty`,`Parish`,`Duration`,`Durationperiod`,`Numberofsamples`) 

values('$sasrmApplctID','$protocol_id','$countryid','$district_id',now(),now(),'$participants','$Municipality','$SubCounty','$Parish','$Duration','$Durationperiod','$Numberofsamples')";
$mysqli->query($sqlA2);
...
$r= $cmdwb->fetch_array();
if($totalStages){
$sqlASubmissionStages="update ".$prefix."submission_stages  set `RecruitmentCountries`='1' where `owner_id`='$sasrmApplctID' and `protocol_id`='$protocol_id'";
...
for ($i=0; $i < count($_POST['gender_id']); $i++) {
$gender_idm=$_POST['gender_id'][$i];
$minimum_agem=$_POST['minimum_age'][$i];
$maximum_agem=$_POST['maximum_age'][$i];
$maximum_age_periodm=$_POST['maximum_age_period'][$i];
$quantity=$_POST['quantity'][$i];

$wm="select * from ".$prefix."study_description_age where  owner_id='$sasrmApplctID' and protocol_id='$submission_id' and gender='$gender_idm' and MinimumAge='$minimum_agem' and MaximumAge='$maximum_agem'";
$cmdwb = $mysqli->query($wm);
$totalStagesGend = $cmdwb->num_rows;
if(strlen($_POST['minimum_age'][$i])>=1 and !$totalStagesGend and $maximum_age_periodm and $maximum_agem and $gender_idm!=1){
$Insert_QR2="insert into ".$prefix."study_description_age (`protocol_id`,`owner_id`,`gender`,`MinimumAge`,`MaximumAge`,`Duration`,`quantity`,`is_sent`) values ('$submission_id','$sasrmApplctID','$gender_idm','$minimum_agem','$maximum_agem','$maximum_age_periodm','$quantity','0')";
$mysqli->query($Insert_QR2);
}

if($gender_idm==1 and !$totalStagesGend){
$Insert_QR2="insert into ".$prefix."study_description_age (`protocol_id`,`owner_id`,`gender`,`MinimumAge`,`MaximumAge`,`Duration`,`quantity`,`is_sent`) values ('$submission_id','$sasrmApplctID','$gender_idm','$minimum_agem','$maximum_agem','$maximum_age_periodm','$quantity','0')";
$mysqli->query($Insert_QR2);
}
...
$sqlA2Protocol="update ".$prefix."submission  set `gender_id`='$gender_id',`recruitment_status_id`='$recruitment_status_id',`study_design`='$study_design',`health_condition`='$health_condition',`sample_size`='$sample_size',`minimum_age`='$minimum_age',`maximum_age`='$maximum_age',`recruitment_init_date`='$recruitment_init_date',`minimum_age_period`='$minimum_age_period',`maximum_age_period`='$maximum_age_period',`riskLevel`='$riskLevel' where id='$submission_id' and owner_id='$sasrmApplctID'";
...
$sqlAMethodology="update ".$prefix."clinical_study_methodology set `general_procedures`='$general_procedures',`analysis_plan`='$analysis_plan',`ethical_considerations`='$ethical_considerations',`interventions`='$interventions',`primary_outcome`='$primary_outcome',`secondary_outcome`='$secondary_outcome',`inclusion_criteria`='$inclusion_criteria',`exclusion_criteria`='$exclusion_criteria' where `protocol_id`='$submission_id' and owner_id='$sasrmApplctID'"; 
...
$sqlstudyData="SELECT * FROM ".$prefix."submission where `owner_id`='$asrmApplctID' and is_sent='0' order by id desc limit 0,1";
$QuerystudyData = $mysqli->query($sqlstudyData);
$rstudyData = $QuerystudyData->fetch_array();

////Insert multiple
$sqlstudyData2="SELECT * FROM ".$prefix."clinical_study_methodology where `owner_id`='$asrmApplctID' and is_sent='0' order by id desc limit 0,1";
$QuerystudyData2 = $mysqli->query($sqlstudyData2);
$rstudyData2 = $QuerystudyData2->fetch_array();

/////study_description_age
/*$sqlAge="SELECT * FROM ".$prefix."study_description_age where `owner_id`='$asrmApplctID' and is_sent='0' and protocol_id='$submission_id' order by id desc limit 0,1";
$QueryAge = $mysqli->query($sqlAge);
$totalStagesAge = $QueryAge->num_rows;

if($totalStages and $rstudyData['study_design'] and $rstudyData['health_condition'] and $rstudyData2['inclusion_criteria'] and $rstudyData2['exclusion_criteria'] and $rstudyData['recruitment_init_date'] and $rstudyData2['interventions'] and $rstudyData2['primary_outcome'] and $rstudyData2['secondary_outcome'] and $totalStagesAge){
$sqlASubmissionStages="update ".$prefix."submission_stages  set `study_description`='1' where `owner_id`='$sasrmApplctID' and `protocol_id`='$submission_id'";
$mysqli->query($sqlASubmissionStages);
}else{echo $sqlASubmissionStages="update ".$prefix."submission_stages  set `study_description`='0' where `owner_id`='$sasrmApplctID' and `protocol_id`='$submission_id'";
$mysqli->query($sqlASubmissionStages);}*/

   
$qRPersoneldSum="select *,sum(quantity) as TotalQuantity from ".$prefix."study_description_age  where owner_id='$sasrmApplctID' and protocol_id='$submission_id' group by protocol_id";
$RPersoneldSum=$mysqli->query($qRPersoneldSum);
$rowRowsQuantity = $RPersoneldSum->fetch_array();
$rowRowsQuantity['TotalQuantity'];
$Allgender=$rowRowsQuantity['gender'];

$sqlSubmissionCountry = "select *,sum(participants) as TotalParticipants FROM ".$prefix."submission_country where owner_id='$sasrmApplctID' and submission_id='$submission_id' group by submission_id";//and conceptm_status='new' 
$resultSubmissionCountry = $mysqli->query($sqlSubmissionCountry);
$rowRowsSubmissionCountry = $resultSubmissionCountry->fetch_array();
$rowRowsSubmissionCountry['TotalParticipants'];

if($rowRowsQuantity['TotalQuantity']==$rowRowsSubmissionCountry['TotalParticipants'] || $Allgender=='1'){
$sqlASubmissionStages="update ".$prefix."submission_stages  set `study_description`='1' where `owner_id`='$sasrmApplctID' and `protocol_id`='$submission_id'";
$mysqli->query($sqlASubmissionStages);
$message='<p class="success">Dear '.$session_fullname.', details have been submitted, proceed to continue</p>';

echo '<meta http-equiv="REFRESH" content="0;url='.$base_url.'/main.php?option=StudyPopulation&id='.$submission_id.'">';
echo '<img src="images/loading_wait.gif">';
echo '<div class="spacer"></div>';

}else{
$sqlASubmissionStages2="update ".$prefix."submission_stages  set `study_description`='0' where `owner_id`='$sasrmApplctID' and `protocol_id`='$submission_id'";
$mysqli->query($sqlASubmissionStages2);
$message="<p class='error2'>Dear '.$session_fullname.', details have been submitted, but Recruitment Areas and Study Participants totals don't match. Please update.</p>";	
...
$sqlmethodology="SELECT * FROM ".$prefix."clinical_study_methodology where `owner_id`='$asrmApplctID' and is_sent='0' order by id desc limit 0,1";
...
$upDelete="delete from ".$prefix."study_description_age  where owner_id='$asrmApplctID' and protocol_id='$protocol_id' and id='$id'";
$mysqli->query($upDelete);

$sqlAgeTotal="SELECT * FROM ".$prefix."study_description_age where `owner_id`='$asrmApplctID' and is_sent='0' and protocol_id='$protocol_id' order by id desc limit 0,1";
$QueryAgeTotal = $mysqli->query($sqlAgeTotal);
$totalStagesAgeTotal = $QueryAgeTotal->num_rows;
if(!$totalStagesAgeTotal){
$sqlASubmissionStages2="update ".$prefix."submission_stages  set `study_description`='0' where `owner_id`='$asrmApplctID' and `protocol_id`='$protocol_id'";
$mysqli->query($sqlASubmissionStages2);	
}
...
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."submission_country where owner_id='$asrmApplctID' and submission_id='$protocol_id' order by id desc LIMIT 0,100";//and conceptm_status='new' 
...
$rmunicipalities=$resultmunicipalities->fetch_array();
	...
	?>
                          <tr>
                            ...
                            <td><input id="go" type="button" value="Update" onclick="window.open('<?php echo $base_url;?>updateareas.php?id=<?php echo $rInvestigator['id'];?>','popUpWindow','height=700, width=1000, left=100, top=100, resizable=yes, scrollbars=yes, toolbar=yes, menubar=no, location=no, directories=no, status=yes');"  class="errorm4" ></td>
   ...
   <?php 
   $totalParticpants=($totalParticpants+$rInvestigator['participants']);
   }///////////end function ?>  
...
<form action="<?php echo $base_url;?>recapprova./main.php?option=submissionThird" method="post" name="regForm" id="regForm" autocomplete="false">                        
 ...
 <?php /*?> <table width="100%" border="0">
  <tr>
    <td width="13%" align="left" valign="top" style="padding-right:15px;">Gender: <span class="error">*</span>
    <select name="gender_id" id="gender_id" class="form-control  required">
    <option value="">------</option>
<?php
$sqlGender = "select * FROM ".$prefix."list_gender order by status asc";//and conceptm_status='new' 
...
<?php }?>
    ...
    <td width="21%" align="left" valign="top" style="padding-right:15px;">Target sample size: <span class="error">*</span>
<?php
$sql3 = "select *,sum(participants) AS participantsm FROM ".$prefix."submission_country where owner_id='$asrmApplctID' and submission_id='$id' group by submission_id order by id desc LIMIT 0,110";//and conceptm_status='new' 
$result3 = $mysqli->query($sql3);
$rInvestigator3=$result3->fetch_array();
$participants=$rInvestigator3['participantsm'];
?>




<input type="text" name="sample_size" id="sample_size" class="form-control  required number" value="<?php if($rstudy['sample_size']){echo $participants;}else{echo $participants;}?>" autocomplete="off" readonly="readonly" placeholder="Add Recruitment Areas up">
</td>
    <td width="23%" align="left" valign="top" style="padding-right:15px;">
    Minimum Age: <span class="error">*</span>
    <input type="text" name="minimum_age" id="minimum_age" class="form-control  required number" value="<?php if($rstudy['minimum_age']){echo $rstudy['minimum_age'];}else{$_POST['minimum_age'];}?>" autocomplete="off">
    
    </td>

    <td width="17%" align="left" valign="top" style="padding-right:15px;">Maximum Age: <span class="error">*</span>
<input type="text" name="maximum_age" id="maximum_age" class="form-control  required number" value="<?php if($rstudy['minimum_age']){echo $rstudy['maximum_age'];}else{$_POST['maximum_age'];}?>"  autocomplete="off">
    
    
</td>
    <td width="14%" align="left" valign="middle" style="padding-right:15px;">
...
<option value="Years" <?php if($rstudy['maximum_age_period']=='Years'){?>selected="selected"<?php }?>>Years</option>
 ...
 <table width="80%" border="0" id="POITable" class="htheadersm">
        <tr>
            ...
            <th width="10%"><strong>Total Number of Participants:</strong></th>
        ...
        <tr>
            ...
            <td>
<input type="hidden" name="dd[]" id="vvv" tabindex="4" class="requiremd"/>
...
<?php }?>
             ...
             <td><input type="text" name="quantity[]" id="quantity" class="form-control number" value=""  autocomplete="off"> </td>
            <td><input type="text" name="minimum_age[]" id="minimum_age" class="form-control number" value="" autocomplete="off"></td>
            
            <td><input type="text" name="maximum_age[]" id="maximum_age" class="form-control number" value=""  autocomplete="off">
            
            </td>

           <td>
           <input type="hidden" name="maximum_age_periodw[]" id="vvv" tabindex="4" class="requiremd"/>
           <select name="maximum_age_period[]" id="maximum_age_period" class="form-control">
    <option value="">------</option>
...
$qRPersoneldSum2="select *,sum(quantity) as TotalQuantity from ".$prefix."study_description_age  where owner_id='$asrmApplctID' and protocol_id='$protocol_id' group by protocol_id";
$RPersoneldSum2=$mysqli->query($qRPersoneldSum2);
$rowRowsQuantity2 = $RPersoneldSum2->fetch_array();
$totalStagesSum2 = $RPersoneldSum2->num_rows;
$rowRowsQuantity2['TotalQuantity'];
$Allgender=$rowRowsQuantity2['gender'];

$sqlSubmissionCountry2 = "select *,sum(participants) as TotalParticipants FROM ".$prefix."submission_country where owner_id='$asrmApplctID' and submission_id='$protocol_id' group by submission_id";//and conceptm_status='new' 
$resultSubmissionCountry2 = $mysqli->query($sqlSubmissionCountry2);
$rowRowsSubmissionCountry2 = $resultSubmissionCountry2->fetch_array();
$rowRowsSubmissionCountry2['TotalParticipants'];
$rowRowsSubmissionCountry2['TotalParticipants'];

if($rowRowsQuantity2['TotalQuantity']!=$rowRowsSubmissionCountry2['TotalParticipants'] and $totalStagesSum2 and $Allgender!='1'){
	
$sqlASubmissionStages2="update ".$prefix."submission_stages  set `study_description`='0' where `owner_id`='$asrmApplctID' and `protocol_id`='$protocol_id'";
$mysqli->query($sqlASubmissionStages2);
echo $message="<p class='error2'>Recruitment Areas and Study Participants totals don't match. Please update.</p>";
}else{
	$sqlASubmissionStages="update ".$prefix."submission_stages  set `study_description`='1' where `owner_id`='$asrmApplctID' and `protocol_id`='$protocol_id'";
$mysqli->query($sqlASubmissionStages2);
...
$qRPersoneld2="select * from ".$prefix."study_description_age  where owner_id='$asrmApplctID' and protocol_id='$protocol_id'";
...
<tr>
            ...
            <td>
              ...
              <input id="go" type="button" value="Update Details" onclick="window.open('<?php echo $base_url;?>studydesc.php?id=<?php echo $rowRows2['id'];?>&pid=<?php echo $protocol_id;?>','popUpWindow','height=700, width=1000, left=100, top=100, resizable=yes, scrollbars=yes, toolbar=yes, menubar=no, location=no, directories=no, status=yes');"  class="errorm4" ><br />
            
            
            <a href="./main.php?option=DelsubmissionThird&id=<?php echo $rowRows2['id'];?>/" style="color:#F00; font-weight:bold;" onclick="return confirm('Are you sure you want to delete this record?');">Delete</a>
...
?> <tr>
            <td id="grid"></td>
           <td id="grid"><b><?php echo $Qty;?></b> </td>
            <td id="grid"></td>
