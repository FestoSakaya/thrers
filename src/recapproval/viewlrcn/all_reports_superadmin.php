...
if($_POST['doSearch']){	
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submission where is_sent='1'  and (created >= '$datesfrom' AND created <= '$datesto') OR status='$status' OR clinical_trial_type='$category' OR paymentStatus='$paymentstatus' OR recAffiliated_id='$recID'  order by id desc");//and conceptm_status='new' 
}
...
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."submission where is_sent='1'  and recAffiliated_id='$recAffiliated_id' and (created >= '$datesfrom' AND created <= '$datesto') OR status='$status' OR clinical_trial_type='$category' OR paymentStatus='$paymentstatus'  OR recAffiliated_id='$recID' order by id desc LIMIT $start, $limitm";//and conceptm_status='new' 
}//
...
<?php if($TotalRevisions){?> <a href="<?php echo $base_url;?>recapprova./main.php?option=RECRevisions&id=<?php echo $rProjects['id'];?>"><span class="label label-sec2">Revisions [<?php echo $TotalRevisions;?>]</span></a>   <?php }?>                
                            </td>
