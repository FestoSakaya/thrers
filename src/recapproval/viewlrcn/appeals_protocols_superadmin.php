...
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submission where is_sent='1' and (public_title like '%$searchprotocol%' OR code like '%$searchprotocol%' OR shtname like '%$searchprotocol%') and status='$status' and  	appeals='Yes' order by id desc");//and conceptm_status='new' 
}

if($_POST['status'] and !$_POST['searchprotocol']){	
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submission where is_sent='1' and  status='$status' and recruitment_status_id='1' order by id desc");//and conceptm_status='new' 
}

if(!$_POST['status'] and $_POST['searchprotocol']){	
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submission where is_sent='1' and  (public_title like '%$searchprotocol%' OR code like '%$searchprotocol%' OR shtname like '%$searchprotocol%') and appeals='Yes' order by id desc");//and conceptm_status='new' 
...
if(!$_POST['doSearch']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submission where is_sent='1' and appeals='Yes' order by id desc");//and conceptm_status='new' 
}
...
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."submission where is_sent='1' and (public_title like '%$searchprotocol%' OR code like '%$searchprotocol%' OR shtname like '%$searchprotocol%') and status='$status' and appeals='Yes' order by id desc LIMIT $start, $limitm";//and conceptm_status='new' 
}//'Pending','reviewed','Approved','Rejected'

if($_POST['status'] and !$_POST['searchprotocol']){							//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."submission where is_sent='1'  and  status='$status' and appeals='Yes' order by id desc LIMIT $start, $limitm";//and conceptm_status='new' 
}//'Pending','reviewed','Approved','Rejected'

if(!$_POST['status'] and $_POST['searchprotocol']){							//if no page var is given, set start to 0
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."submission where is_sent='1'  and (public_title like '%$searchprotocol%' OR code like '%$searchprotocol%' OR shtname like '%$searchprotocol%') and appeals='Yes' order by id desc LIMIT $start, $limitm";//and conceptm_status='new' 
}//'Pending','reviewed','Approved','Rejected'

if(!$_POST['doSearch']){
$sql = "select *,DATE_FORMAT(`created`,'%d/%m/%Y %H:%s:%i') AS created FROM ".$prefix."submission where is_sent='1' and appeals='Yes' order by id desc LIMIT $start, $limitm";//and conceptm_status='new'
}
...
<?php }?>
  ...
  <?php if($sqUserddRREC3['appealSubmitted']=='Yes' and $sqUserddRREC3['status']=='Pending'){?>
  <a href="main.php?option=AssignReviewersAppeals&id=<?php echo $rProjects['id'];?>&act=<?php echo $sqUserddRREC3['id'];?>"><span class="label label-sec">Assign Reviewers</span></a><div style="margin-bottom:6px;"></div>
  
  <?php }?>
  
  
  <?php /*?><a href="./main.php?option=viewsubmissionrec/<?php echo $rProjects['id'];?>/"><span class="label label-sec">View Submission</span></a>
<div style="margin-bottom:6px;"></div>
 <a href="./main.php?option=viewcomments/<?php echo $rProjects['id'];?>/"><span class="label label-sec">View Comments</span></a>                        
