...
$sqlprotocalAttachments="SELECT * FROM ".$prefix."ammendments_documents where atype='$file_type' and owner_id='$sessionasrmApplctID' and is_sent='0' and fileAttachment='$fileattachment' and amendment_id='$id' order by id desc";
$QprotocalAttachments = $mysqli->query($sqlprotocalAttachments);
$totalstudyAttachments = $QprotocalAttachments->num_rows;

if(!$totalstudyAttachments and $id){
...
values('$sessionasrmApplctID','$id','$recAffiliated_id','','$fileattachment','$file_type',now(),'Pending','Not Assigned','','','$Language','$Version','$Date','$code','$ReasonforAmendment','','','','','','','','Not Paid','0','manual','','','$id')";
$mysqli->query($sqlA2);
}
if($totalstudyAttachments){

$sqlA2="update ".$prefix."ammendments_documents  set `fileAttachment`='$fileattachment',`aLanguage`='$Language',`aVersion`='$Version',`aDate`='$Date' where `owner_id`='$sessionasrmApplctID' and atype='$file_type' and is_sent='0' and amendment_id='$id'";
$mysqli->query($sqlA2);
}
...
if($_POST['doSubmitAmendment']=='Click to Finish and Submit Amendments' and $id){
...
$sqlprotocalSubSelCAll2="SELECT * FROM ".$prefix."ammendments where owner_id='$sessionasrmApplctID'  and id='$id' order by id desc";
...
$sqlA2Protocol12="update ".$prefix."ammendments_documents  set `is_sent`='1' where `owner_id`='$sessionasrmApplctID' and amendment_id='$id'";
$mysqli->query($sqlA2Protocol12); 

$sqlA2Protocol1="update ".$prefix."ammendments  set `status`='Submitted',`is_sent`='1',`CompletenessCheck`='Pending' where `owner_id`='$sessionasrmApplctID' and id='$id'";
...
$mail->Subject = "Ammendments Confirmation - $protocol_title";
$body="$allSentMessage
";
$mail->MsgHTML($body);
...
Thank you for your renewal for protocol titled, '<b>$protocol_title</b>' Your Ammendmnets have been submitted on <b>$today</b>.<br /><br />
...
echo '<meta http-equiv="REFRESH" content="2;url='.$base_url.'/main.php?option=MyAmmendments&id=$id">';
...
<?php if($totalstudyAmmendment>=1){?><li><a href="./main.php?option=ManualAmmendmentsSec&id=<?php echo $id;?>" <?php if($totalstudyAttachments>=1){?> style="background:green; color:#FFF;" <?php }?>>Attachments</a></li><?php }?>
                  ...
                  <div class="left-col col-lg-6 d-flex align-items-center justify-content-between">
                    <div class="project-title d-flex align-items-center">
                     <?php if($sqUserdd['profile']){?> <div class="image has-shadow"><img src="files/profile/<?php echo $sqUserdd['profile'];?>" alt="..." class="img-fluid"></div><?php }?>
                      <div class="text">
                        <h3 class="h4">Protocal Title</h3><small><?php echo $rstudymAmmendment['protocol_title'];?></small>
                      </div>
                    </div>
...
$sqlstudy="SELECT *,DATE_FORMAT(`created`,'%Y-%m-%d') AS created FROM ".$prefix."ammendments_documents where `owner_id`='$sessionasrmApplctID' and is_sent!='1' and atype='Payment' and amendment_id='$id' order by id desc";//and is_sent!='1' 
...
//////////////Get Totals
$wmTrackedChanges="select * from ".$prefix."ammendments_documents where  `owner_id`='$sessionasrmApplctID' and atype='TrachedChanges'  and amendment_id='$id' order by id desc limit 0,1";
$cmdwbTrackedChanges = $mysqli->query($wmTrackedChanges);
$totalsTrackedChanges = $cmdwbTrackedChanges->num_rows;

$wmCleanCopy="select * from ".$prefix."ammendments_documents where  `owner_id`='$sessionasrmApplctID' and atype='CleanCopy'  and amendment_id='$id' order by id desc limit 0,1";
$cmdwbCleanCopy = $mysqli->query($wmCleanCopy);
$totalsCleanCopy = $cmdwbCleanCopy->num_rows;

$wmCoverLetter="select * from ".$prefix."ammendments_documents where  `owner_id`='$sessionasrmApplctID' and status='Pending' and atype='CoverLetter'  and amendment_id='$id' order by id desc limit 0,1";
...
$wmPayment="select * from ".$prefix."ammendments_documents where  `owner_id`='$sessionasrmApplctID' and atype='Payment'  and amendment_id='$id' order by id desc limit 0,1";
...
<form action="" method="post" name="regForm" id="regForm" autocomplte="false">
 ...
 <input name="protocol_title" type="hidden" value="<?php echo $rRenewals['protocol_title'];?>"/>
